...
1. **`crm_records.portalCollections`** (CRM → Portal Access → Available Collections) — the customer's allowed collections. Empty array = no restriction, which is why shipping the field changed nothing for existing customers. **The portal must enforce it server-side** (`functions/portalEngine.js` + the `sanitizeStep()` whitelist), exactly like `portalFlowIds`. Until then it is an internal note only: a customer's portal still shows every entitled flow regardless of collection.
