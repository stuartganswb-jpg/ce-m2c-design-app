...
const LibraryTab = ({ currentUser, activeBrand }) => {
  ...
  const [newFinishConfig, setNewFinishConfig] = useState({ name: '', code: '', type: 'PLATING', textureUrl: '' });
  ...
  // 1. Fetch Dynamic Schema & Global Finishes
  useEffect(() => {
    // Listen to Schema
  ...
  // Safe handler for deeply nested custom fields
  ...
  const handleCreateNewPart = () => {
    ...
    setActivePart({
      isNew: true, id: newId, itemId: newId, legacyErpId: "PENDING", itemName: "NEW STANDALONE COMPONENT",
      brandId: activeBrand, partClass: "Inventory",
    });
    setEditSpecs({
      productType: "COMPONENT", uom: "EA", finishDetail: "", collection: "N/A", watchList: "NONE",
  ...
  const handleRemoveSchemaField = async (keyToRemove) => {
      if (!window.confirm("Remove this field? Existing data on parts will remain in DB but be hidden from UI.")) return;
  ...
  const handleAddGlobalFinish = async () => {
      ...
      try {
          ...
          setNewFinishConfig({ name: '', code: '', type: 'PLATING', textureUrl: '' });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px', fontFamily: 'monospace', backgroundColor: '#e5e5e5', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#fff', border: '2px solid #000', padding: '15px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '5px 5px 0 #000' }}>
        ...
        <div style={{ display: 'flex', gap: '15px', width: '65%', alignItems: 'center' }}>
          ...
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} style={{ padding: '10px', border: '2px solid #000', fontWeight: 'bold', flex: 1 }}><option value="">ALL CATEGORIES</option>{PROD_TYPES.map(pt => <option key={pt} value={pt}>{pt}</option>)}</select>
      ...
      <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: activePart ? 1.5 : 1, display: 'grid', gridTemplateColumns: activePart ? 'repeat(auto-fill, minmax(200px, 1fr))' : 'repeat(auto-fill, minmax(250px, 1fr))', gap: '15px', alignContent: 'start' }}>
          {filteredInventory.map(part => {
            ...
            return (
              <div key={part.id} onClick={() => openPartDetails(part)} style={{ background: activePart?.id === part.id ? '#fff9c4' : '#fff', border: activePart?.id === part.id ? '3px solid #000' : '2px solid #ccc', cursor: 'pointer', position: 'relative', display: 'flex', flexDirection: 'column', transition: '0.2s', boxShadow: activePart?.id === part.id ? '5px 5px 0 #000' : 'none' }}>
                ...
                <div style={{ height: '150px', background: '#f4f4f4', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '15px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #eee', paddingTop: '10px', marginTop: '10px' }}><span style={{ fontSize: '0.65rem', background: '#eee', padding: '3px 6px', borderRadius: '3px' }}>{specs.productType || "NO TYPE"}</span><span style={{ fontSize: '0.65rem', fontWeight: 'bold' }}>{specs.isInHouse ? "IN-HOUSE" : "VENDOR"}</span></div>
                </div>
              </div>
            );
          })}
            ...
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div>
                 ...
                 <div style={{ display: 'flex', gap: '10px' }}>
                   <div style={{ flex: 2 }}><label style={{ fontSize: '0.65rem', fontWeight: 'bold' }}>PART NAME / DESCRIPTION:</label><input name="tempName" value={editSpecs.tempName !== undefined ? editSpecs.tempName : activePart.itemName} onChange={handleSpecChange} style={{ width: '100%', padding: '8px', border: '1px solid #000', boxSizing: 'border-box', fontWeight: 'bold' }} /></div>
                 ...
                 <div style={{ marginTop: '15px', background: '#f4f4f4', padding: '10px', border: '1px solid #ccc' }}>
                                ...
                                {field.type === 'dropdown' ? (
                                    ...
                                    <div style={{ background: '#fff', border: '2px solid #007bff', padding: '10px' }}>
                                        {editSpecs.customData?.[field.key] && (
                                            <div style={{ marginBottom: '8px', paddingBottom: '8px', borderBottom: '1px dashed #ccc' }}>
              ...
              <div>
                 ...
                 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                   ...
                   {/* DYNAMIC GLOBAL FINISHES DROPDOWN */}
                   <div>
                       ...
                       <select name="finishDetail" value={editSpecs.finishDetail || ""} onChange={handleSpecChange} style={{ width: '100%', padding: '8px', border: '2px solid #6f42c1', outline: 'none' }}>
                           ...
                           <option value="N/A">N/A (Raw/Unfinished)</option>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {editSpecs.isInHouse ? (
                    <>
                      ...
                      <div style={{ borderTop: '2px dashed #ccc', paddingTop: '10px', marginTop: '5px' }}>
                        <label style={{ fontSize: '0.7rem', fontWeight: 'bold', display: 'block', marginBottom: '5px' }}>UPLOAD PRINT (PDF):</label>
                        {editSpecs.pdfUrl && <a href={editSpecs.pdfUrl} target="_blank" rel="noreferrer" style={{ fontSize: '0.7rem', color: '#007bff', display: 'block', marginBottom: '5px' }}>[View Current PDF]</a>}
              ...
              {/* CAD RULES */}
              <div style={{ background: '#f8f9fa', border: '1px solid #007bff', padding: '15px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <div style={{ gridColumn: 'span 2' }}><label style={{ fontSize: '0.65rem', fontWeight: 'bold', color: '#d9534f' }}>Z-INDEX / RENDER LAYER:</label><input name="layeringSequence" type="number" step="10" value={editSpecs.layeringSequence || ""} onChange={handleSpecChange} placeholder="e.g. 10 (Back), 30 (Front)" style={{ width: '100%', padding: '8px', border: '1px solid #ccc', boxSizing: 'border-box' }} /></div>
                  <div style={{ gridColumn: 'span 2', background: '#e3f2fd', padding: '10px', border: '1px dashed #007bff' }}><label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#007bff' }}>WIDTH OFFSET / DEDUCTION (INCHES):</label><input name="widthOffset" type="number" step="0.125" value={editSpecs.parametric?.widthOffset || ""} onChange={handleParametricChange} style={{ width: '100%', padding: '8px', border: '1px solid #007bff', boxSizing: 'border-box', fontWeight: 'bold' }} /></div>
                </div>
              </div>

              <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                <button onClick={savePartUpdates} style={{ flex: 2, padding: '15px', background: isSaving ? '#28a745' : '#000', color: '#fff', fontWeight: 'bold', fontSize: '1rem', border: 'none', cursor: 'pointer', transition: '0.3s' }}>{isSaving ? "SAVED ✓" : "SAVE PART"}</button>
                {!activePart.isNew && <button onClick={handleDeletePart} style={{ flex: 1, padding: '15px', background: '#fff', color: '#d9534f', border: '2px solid #d9534f', fontWeight: 'bold', cursor: 'pointer' }}>🗑️ DELETE</button>}
              </div>
      ...
      {/* ========================================================= */}
      {/* ADMIN SCHEMA & FINISH MANAGER                             */}
      {/* ========================================================= */}
      {isAdmin && (
          <>
            {/* 1. DYNAMIC FIELD MANAGER */}
            <div style={{ marginTop: '40px', background: '#fff', border: '3px dashed #17a2b8', display: 'flex', flexDirection: 'column', boxShadow: '10px 10px 0 #17a2b8' }}>
                <div style={{ padding: '15px', background: '#17a2b8', color: '#fff', fontWeight: 'bold', fontSize: '1.2rem', display: 'flex', justifyContent: 'space-between' }}><span>⚙️ DYNAMIC FIELD MANAGER (ADMIN)</span></div>
                <div style={{ padding: '20px', display: 'flex', gap: '20px' }}>
                    <div style={{ flex: 1, borderRight: '2px solid #eee', paddingRight: '20px' }}>
                        <h4 style={{ margin: '0 0 15px 0', borderBottom: '2px solid #000', paddingBottom: '5px' }}>ADD NEW GLOBAL FIELD</h4>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            <div><label style={{ fontSize: '0.7rem', fontWeight: 'bold' }}>FIELD LABEL:</label><input value={newFieldConfig.label} onChange={(e) => setNewFieldConfig({...newFieldConfig, label: e.target.value})} placeholder="e.g. CPQ Front Render" style={{ width: '100%', padding: '10px', border: '1px solid #000', boxSizing: 'border-box' }} /></div>
                            <div>
                                <label style={{ fontSize: '0.7rem', fontWeight: 'bold' }}>DATA TYPE:</label>
                                <select value={newFieldConfig.type} onChange={(e) => setNewFieldConfig({...newFieldConfig, type: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #000', boxSizing: 'border-box' }}>
                                    <option value="text">Text (String)</option><option value="number">Number</option><option value="dropdown">Dropdown (Select)</option>
                                    <option value="file">File Upload (PNG, GLB, PDF)</option>
                                </select>
                            </div>
                            {newFieldConfig.type === 'dropdown' && <div><label style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#007bff' }}>DROPDOWN OPTIONS (Comma Separated):</label><input value={newFieldConfig.options} onChange={(e) => setNewFieldConfig({...newFieldConfig, options: e.target.value})} placeholder="e.g. Class A, Class B" style={{ width: '100%', padding: '10px', border: '1px solid #007bff', boxSizing: 'border-box' }} /></div>}
                            <button onClick={handleAddSchemaField} style={{ padding: '12px', background: '#17a2b8', color: '#fff', fontWeight: 'bold', border: 'none', cursor: 'pointer', marginTop: '10px' }}>+ INJECT NEW FIELD</button>
                        </div>
                    </div>
                    <div style={{ flex: 1 }}>
                        <h4 style={{ margin: '0 0 15px 0', borderBottom: '2px solid #000', paddingBottom: '5px' }}>ACTIVE CUSTOM FIELDS</h4>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '300px', overflowY: 'auto' }}>
                            {customSchema.length === 0 && <span style={{ color: '#999', fontStyle: 'italic' }}>No custom fields added yet.</span>}
                            {customSchema.map(field => (
                                <div key={field.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8f9fa', padding: '10px', border: '1px solid #ccc' }}>
                                    <div><div style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>{field.label}</div><div style={{ fontSize: '0.65rem', color: '#666' }}>Type: {field.type} | ID: {field.key}</div></div>
                                    <button onClick={() => handleRemoveSchemaField(field.key)} style={{ background: 'none', border: 'none', color: '#d9534f', fontSize: '1rem', cursor: 'pointer' }}>🗑️</button>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div style={{ marginTop: '40px', background: '#fff', border: '3px dashed #6f42c1', display: 'flex', flexDirection: 'column', boxShadow: '10px 10px 0 #6f42c1' }}>
                <div style={{ padding: '15px', background: '#6f42c1', color: '#fff', fontWeight: 'bold', fontSize: '1.2rem', display: 'flex', justifyContent: 'space-between' }}><span>🎨 GLOBAL FINISHES & TEXTURES (ADMIN)</span></div>
                <div style={{ padding: '20px', display: 'flex', gap: '20px' }}>
                    
                    <div style={{ flex: 1, borderRight: '2px solid #eee', paddingRight: '20px' }}>
                        <h4 style={{ margin: '0 0 15px 0', borderBottom: '2px solid #000', paddingBottom: '5px' }}>CREATE NEW FINISH RECIPE</h4>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            <div><label style={{ fontSize: '0.7rem', fontWeight: 'bold' }}>FINISH NAME:</label><input value={newFinishConfig.name} onChange={(e) => setNewFinishConfig({...newFinishConfig, name: e.target.value})} placeholder="e.g. Matte Brass" style={{ width: '100%', padding: '10px', border: '1px solid #000', boxSizing: 'border-box' }} /></div>
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <div style={{ flex: 1 }}><label style={{ fontSize: '0.7rem', fontWeight: 'bold' }}>CODE (Opt):</label><input value={newFinishConfig.code} onChange={(e) => setNewFinishConfig({...newFinishConfig, code: e.target.value})} placeholder="e.g. MB" style={{ width: '100%', padding: '10px', border: '1px solid #000', boxSizing: 'border-box' }} /></div>
                                <div style={{ flex: 1 }}><label style={{ fontSize: '0.7rem', fontWeight: 'bold' }}>TYPE:</label><select value={newFinishConfig.type} onChange={(e) => setNewFinishConfig({...newFinishConfig, type: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #000', boxSizing: 'border-box' }}><option value="PLATING">Plating/Metal</option><option value="PAINT">Paint</option><option value="WOOD">Wood Stain</option></select></div>
                            </div>
                            
                            {/* SEAMLESS TEXTURE UPLOAD */}
                            <div style={{ background: '#f4f4f4', padding: '15px', border: '2px dashed #ccc', marginTop: '5px' }}>
                            ...
                            <button onClick={handleAddGlobalFinish} style={{ padding: '12px', background: '#6f42c1', color: '#fff', fontWeight: 'bold', border: 'none', cursor: 'pointer', marginTop: '10px' }}>+ SAVE FINISH RECIPE</button>
                        </div>
                    </div>

                    <div style={{ flex: 1 }}>
                        <h4 style={{ margin: '0 0 15px 0', borderBottom: '2px solid #000', paddingBottom: '5px' }}>MASTER FINISH DICTIONARY</h4>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '350px', overflowY: 'auto' }}>
                            ...
                            {globalFinishes.map(finish => (
                                <div key={finish.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8f9fa', padding: '10px', border: '1px solid #ccc', borderLeft: `5px solid #6f42c1` }}>
                                    <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                                        {/* Texture Thumbnail */}
                                        <div style={{ width: '40px', height: '40px', background: finish.textureUrl ? `url(${finish.textureUrl}) center/cover` : '#ccc', borderRadius: '50%', border: '2px solid #fff', boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }} />
                                        <div>
                                            <div style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>{finish.name} {finish.code && `(${finish.code})`}</div>
                                            <div style={{ fontSize: '0.65rem', color: '#666' }}>Type: {finish.type}</div>
                                        </div>
                                    </div>
                                    <button onClick={() => handleRemoveFinish(finish.id)} style={{ background: 'none', border: 'none', color: '#d9534f', fontSize: '1rem', cursor: 'pointer' }}>🗑️</button>
                                </div>
                            ))}
                        </div>
                    </div>
