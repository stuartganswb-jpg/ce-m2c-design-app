                                    ...
                                    : breakdown.map((l, i) => {
                                        ...
                                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', padding: '5px 0', borderBottom: '1px dashed var(--line)' }}>
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                {code && <div style={{ fontFamily: 'var(--mono)', fontSize: '0.75rem', fontWeight: 700, color: 'var(--ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{code}{l.clientSku && l.clientSku !== code ? <span style={{ fontWeight: 400, color: 'var(--brass)' }}>{` · ${l.clientSku}`}</span> : null}</div>}
                                                ...
                                                {(l.partHandling || l.cutLength || l.finishLabel || l.finishCode) && <div style={{ fontFamily: 'var(--mono)', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--ink-soft)', marginTop: '2px' }}>{l.partHandling ? `→ ${l.partHandling}` : ''}{l.cutLength ? ` · cut ${l.cutLength}"` : ''}{l.perFoot && Number(l.feet) > 0 ? ` · ${l.feet} ft billed` : ''}{(l.finishLabel || l.finishCode) ? <span style={{ color: 'var(--brass)' }}>{` · finish ${l.finishLabel || l.finishCode}${l.clientFinishName ? ` · ${l.clientFinishName}` : ''}`}</span> : ''}</div>}
