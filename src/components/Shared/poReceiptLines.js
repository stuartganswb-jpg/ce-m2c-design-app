...
export const poLinesSql = (nsPoId) => {
    ...
    return `SELECT tl.id AS line, tl.item AS item_internal, UPPER(i.itemid) AS itemid, `
...
export const nsPoLinesOf = (rows) => (Array.isArray(rows) ? rows : []).map(r => ({
    ...
    ordered: N(r.ordered), done: N(r.done), closed: truthy(r.isclosed),
})).filter(l => l.lineId);
...
export function itemReceiptItemsOf({ poItems, nsLines, applied, bin = '' }) {
    const ns = Array.isArray(nsLines) ? nsLines : [];
