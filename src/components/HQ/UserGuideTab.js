...
const WorkOrdersGuide = () => (
    <div>
        ...
        <div style={{ overflowX: 'auto' }}>
            <table style={S.table}>
                ...
                <tbody>
                    ...
                    <tr><td style={S.td}>Build raw mill cores (H1 bases) for the shop</td><td style={S.td}><b>12.5 Stock View → RAW view</b></td><td style={S.td}>Cores route to the SHOP. The RAW generator knows that; the FIN snapshot does not (see edges below).</td></tr>
                    ...
                    <tr><td style={S.td}>Run ONE item now, from its card, with a live component check</td><td style={S.td}><b>4. Master Library</b></td><td style={S.td}>The only path that checks live NetSuite component stock before creating, and offers make-up orders for shorts.</td></tr>
        ...
        <div style={{ overflowX: 'auto' }}>
            <table style={S.table}>
                ...
                <tbody>
                    ...
                    <tr><td style={S.td}>Stock View · WO grid</td><td style={S.td}>advisory suggestions only</td><td style={S.td}>—</td></tr>
                    ...
                    <tr><td style={S.td}>Stock View · RAW / TIER</td><td style={S.td}>advisory + tier warning</td><td style={S.td}>warns in the confirm; you order the base yourself</td></tr>
                    <tr><td style={S.td}>Sales Snapshot</td><td style={S.td}>live sales + availability drive the recommendation</td><td style={S.td}>no component check on the parked WOs — the floor's pick surfaces it</td></tr>
                </tbody>
            </table>
        </div>
        ...
        <ul style={{ paddingLeft: '4px', listStyle: 'none' }}>
            <li style={S.edge}>• <b>Order raw cores from the RAW or TIER view, not the FIN snapshot.</b> The FIN snapshot routes everything it makes to the finishing floor — a raw core ordered there arrives as a finishing job with no recipe.</li>
            <li style={S.edge}>• <b>Order /P converts from the TIER view.</b> The same /P item ordered from the FIN snapshot becomes a finishing WO instead of a Convert to-do.</li>
...
const OrdersCustomersGuide = () => (
    <div>
        ...
        <Screen title="The flow, start to saved" tag="configure · price · quote">
            ...
            <Path name="6 · Cart">Add to quote, then configure the next room — each line keeps its own sidemark and qty; <b>Edit</b> reopens a line, trade discounts (from the customer's CRM discount code, Standard level only, items only) show per line.</Path>
            <Path name="7 · Checkout">Verify customer → shipping (saved NetSuite address or custom drop-ship, optional shipping $ that lands on the NetSuite header) → job name, sidemark, customer PO #, internal memo (never customer-facing) → <b>Add-ons &amp; fees</b> (rush, packaging, strike-offs — each becomes its own line; percentage fees compute off the configured subtotal, so they never compound).</Path>
            <Path name="8 · Save = send" goes="CRM pipeline · RTG board · NetSuite — automatically"><b>💾 Save as Quote</b> puts it on the customer's pipeline and queues the NetSuite estimate; <b>🛒 Save as Sales Order</b> also puts it on the RTG board and queues the NetSuite SO. The real numbers write back onto the record within ~a minute. No approve-then-push relay — saving is sending.</Path>
            <Path name="9 · Documents">A print window opens with the Quotation, the Factory Router (BOM), and one Engineering Drawing page per Vision drawing. Drawings also file to the customer's CRM record.</Path>
        ...
        <Screen title="The counter" tag="stock · to-be-finished · kits · fees">
            ...
            <Path name="Invoices">The order captures its customer-facing invoice at save (kits as one priced line with unpriced components). In CRM, <b>🧾 Invoice → Match NetSuite Invoice #</b> ties it 1:1 to the NetSuite bill before sending.</Path>
        </Screen>
