...
const WorkOrdersGuide = () => (
    <div>
        ...
        <p style={S.p}>When Auto-Release is ON (the ⚡ button on RTG Dispatch), newly created orders push themselves to the floors <b>one at a time</b>, in arrival order — sales orders auto-split, stock WOs follow their stated route, everything logged on the board and the Daily Job Log. Orders that need a human still wait: anything stopped, or created before the switch was flipped, stays parked for a manual push; a gated order (rod cut, convert, components still milling) releases itself the moment its gate clears. Since 2 Sep no stock order is "ambiguous about its route" — every writer states it. RTG stays the record and the control — turn the ⚡ off any time to go back to manual.</p>
...
const OrdersCustomersGuide = () => (
    <div>
        ...
        <Screen title="The counter" tag="stock · to-be-finished · kits · fees">
            ...
            <Path name="To-be-finished → floor" goes="in-house → RTG finishing WO · outsourced → WMS Plating (new 27 Aug)">A sales order's <b>To Be Finished</b> lines fire their own floor work at save, routed by the finish itself. An <b>in-house finish</b> → a finishing work order parked in RTG under the customer's name, recipe = the chosen finish, pulling the /P of each component exactly like a CPQ custom, with the <b>component pre-check</b> (short /P → Convert to-do + gate; short raw → component shop WO); each WO waits <b>⏳ AWAITING SO ACCEPT</b> until NetSuite takes the sales order — a rejected order never becomes work. An <b>outsourced finish</b> (the Outsource Finishes tab is the authority) never becomes a finishing WO: per mill core, in stock → a <b>plating demand</b> on the WMS Plating tab; core short → a shop WO to mill it first, plate after. Never order these from Stock View or the snapshot: those screens see BOM demand, not the finish. On the WMS Stock card those lines read <b>FROM FINISHING / FROM PLATING — do not pull raw</b>.</Path>
