...
const WorkOrdersGuide = () => (
    <div>
        ...
        <Screen title="Generate Production Work Order" tag="five distinct paths">
            ...
            <Path name="Just For Paint" goes="Finishing floor (direct) · existence check only">For an item the app was never taught — a legacy NetSuite code with no assembly. Only on the JFP template card; see below.</Path>
            <Path name="♻ Repaint from another finish" goes="Finishing floor (direct) + WMS pick · live stock, blocking (new 8–9 Sep)">On <b>any item carrying a finish suffix</b>, under <em>Generate Production Work Order</em> — and on the <b>Sales Snapshot</b> row beside the ✂ rod cut, where the shortage is actually seen. Short of HHRMBF75/<b>M3</b> but holding /<b>M1</b>? Pull the M1, paint it, and NetSuite adjusts the pulled colour <b>down</b> and the ordered colour <b>up</b>. <b>No BOM is needed</b>, which is the point: it is faster and cleaner than teaching the app an assembly for something we simply repaint. It lists that item's other colours <em>from NetSuite</em> with live available quantities, sorted so the ones that can cover the order come first (empty colours are shown greyed, not hidden), and you can type any other item # instead. <b>A colour without the stock is refused, not warned</b> — a repaint against stock that is not there fails at the pick two weeks later. It is offered whatever the sourcing tag says: an outsourced finish we happen to be able to paint here is exactly the case it exists for.</Path>
        </Screen>
        ...
        <Screen title="The builders" tag="type quantities, press once">
            ...
            <Path name="Stock Build Needs" goes="bought shortages on stock runs → the vendor's open draft PO">When a stock run's pre-check finds a <b>bought</b> component short, the run is still created — a purchase order is a person's decision — and the shortage is now recorded <b>on the work order</b> instead of passing once through a log. This view lists them <b>grouped by vendor</b>, with one <em>Add to PO</em> per vendor that joins their open draft (step 1b) and stamps the PO back onto every work order that contributed. A line already on a live PO reads as covered; a closed or deleted PO does not count. An item with no vendor says so rather than failing at the press.</Path>
        ...
        <Screen title="Generate Orders" tag="recommendation → one pass">
            ...
            <Path name="BO — who is actually waiting" goes="(new 9 Sep)">The red number beside <b>Avail</b> counts pieces on <b>open sales orders that nothing on the shelf can make</b>. This is <em>our</em> record, not NetSuite's <code>quantitybackordered</code> — NetSuite counts what its sales orders could not commit, including lines we have already covered another way. Hover for the orders and the customers; click for the <b>📋 Backorders</b> board: oldest order first, what covers each line, and — in red — the ones <b>nobody has ordered</b>.</Path>
            <Path name="Display — the boards that will pull this item" goes="(new 12 Sep)">The purple number beside <b>BO</b> is the open <b>sales display board</b> demand for the row — display orders are lump-sum items in NetSuite, so <em>committed</em> never sees them, and the build-order panel on 5. Marketing keeps this figure current from every open display order. Hover it for the orders behind it. It counts in <b>Rec</b> exactly as backorder does, so a 50-board pull raises the suggestion before the picker hits the shelf.</Path>
            ...
            <Path name="A component NetSuite has no stock row for" goes="(new 12 Sep)">The pre-check treats a part NetSuite returned nothing for as <b>unknown, not zero</b>: the line reads "NO NETSUITE STOCK ROW — CHECK THE ITEM" and is <em>not</em> counted as short, so no make-up work order or convert is raised for something that may be on the shelf. A shortage is only ever a number NetSuite gave us.</Path>
            <Path name="Sourcing decides — and IN-HOUSE means made" goes="one rule, every view (new 9 Sep)"><b>In-house stays a work order</b>, whatever vendor the item names. A vendor on the record only says who <em>could</em> supply it; if we genuinely make it <b>and</b> buy it, set the item to <b>BOTH</b> in the Master Library — that is what BOTH is for, and it is the only way to say it. <b>Outsourced</b> buys (and is held, never guessed, when no vendor is set). <b>BOTH</b> asks. The only other item that still asks is one whose sourcing has <em>never been set</em> — in-house by the old flag but carrying a vendor. <b>Every ask opens on the WORK ORDER</b>: doing nothing has to produce the recoverable answer, because an unwanted work order parks in RTG and an unwanted PO is money. <em>(Until 9 Sep an in-house item that merely named a supplier opened pre-selected to PO — pressing through bought something we make.)</em></Path>
        ...
        <div style={{ overflowX: 'auto' }}>
            <table style={S.table}>
                ...
                <tbody>
                    ...
                    <tr><td style={S.td}><b>1b · Add to it</b> <span style={S.tabno}>(new 8 Sep)</span></td><td style={S.td}>A later need for the same vendor <b>joins the open draft</b> rather than starting a second PO — which is what makes a vendor minimum reachable instead of being split across five small orders nobody can send. The same item wanted by the <em>same</em> sales order adds to that line's quantity; wanted by a <em>different</em> one it gets its own line, because the line is what says who the parts are for. If the vendor's last PO has already gone to NetSuite, a <b>new draft</b> is started instead — see the rule below.</td><td style={S.td}>anywhere that buys, and <b>🧱 Stock Build Needs</b></td></tr>
                    ...
                    <tr><td style={S.td}><b>5 · Acknowledge</b></td><td style={S.td}>Days later the vendor confirms, usually with a ready date. Record it on the PO header: their order number, their ready date, a note. That date is the <em>vendor's</em> commitment and is kept separate from the ready date we promise a customer. <b>The acknowledgement is the final confirmation</b> — see below.</td><td style={S.td}>the same card</td></tr>
                </tbody>
            </table>
        </div>
        <div style={S.note}><b>A draft that is never approved can be discarded.</b> <span style={S.tabno}>(new 9 Sep)</span> <em>✕ Discard</em> on the PO in the review window. The record is <b>kept</b> — stamped deleted, dated, with your name and reason — and drops off every screen at once: the board, the draft badge, and the open PO that later needs would otherwise have been added to. That last one matters: since drafts accumulate (step 1b), a stale draft is not inert, it is a live target. Discard is <b>refused</b> once the PO has a NetSuite number — those are closed in NetSuite and marked here, so the two never disagree.</div>
        <div style={S.note}><b>A purchase order is FINAL once it has left us.</b> <span style={S.tabno}>(Stuart, 8 Sep)</span> Lines are added and changed freely while it is a <b>draft</b> — that is the whole point of step 1b, and of vendor minimums. But the moment it goes to NetSuite for its number, our copy must not drift from the one over there; once it is emailed, the vendor is quoting and scheduling against exactly what they were sent; and once they acknowledge it, it is a mutual record. From the <b>first</b> of those steps onward <b>nothing can be added or changed</b> — not a quantity, not a line. Anything further is a <b>new purchase order</b>: the vendor needs to see a new document, not a quietly different one. Receiving, delivery notes and the acknowledgement itself all still work — those are how a finished PO keeps moving, and none of them changes what was ordered.</div>
        ...
        <h2 style={S.h2}>⚡ Auto-Release — RTG is the record and the control <span style={S.tabno}>(RTG 13, rebuilt 4 Sep · 8 Sep)</span></h2>
        <p style={S.p}><b>Every order from every door lands on RTG Dispatch</b> — CPQ, Order Entry, Stock View, the Snapshot, the Library — and RTG sends it on by itself. There are <b>no Push buttons</b>. The ⚡ button on the RTG toolbar is the <b>kill switch</b>: ON, every approved record whose gates are clear goes to its floor <b>one at a time</b>, through the door its type names (a CPQ sales order splits to shop + finishing + packaging; a stock work order goes to finishing <b>and queues its NetSuite work order at the same moment</b>; a shop route goes to the shop; an Order Entry line goes to finishing with the sales order as its NetSuite record). OFF, nothing releases and every chip says so. Turning it ON does not flood the floor: orders parked <em>before</em> the switch wait for a person.</p>
        <p style={S.p}><b>Read the chip, don't hunt for a button.</b> Each parked row carries one chip in the same words everywhere: <em>awaiting SO accept · awaiting NetSuite WO # · awaiting component milling · awaiting phosphate convert · awaiting rod cut · awaiting material</em> (the gates — each clears itself when the WMS, the shop, receiving or NetSuite does its part), <em>waiting for the rest of the order</em> (see Finish complete, below), or why nothing will take it (<em>auto-release is OFF · parked before the switch · stopped · no route on this record</em>), or <em>releasing…</em>. Every parked record names its <b>source</b> (which screen parked it) and its route; a record with no source is a red line — report it.</p>
        ...
        <p style={S.p}><b>Finish complete, or finish as available.</b> The default is to <b>wait and finish an order complete</b> when every part is in: a sales order's work orders wait for each other, and an order the split found short is written to the Setup Queue <b>on hold</b>, naming the missing material. The exception is the flag <b>⚡ Finish as available</b> on the sales order — on the RTG card or the WMS SO Pack card, with a reason, logged with who and when. Turning it on lets the in-stock parts run now; the late parts follow on the same order. Turning it off never stops work already on the floor.</p>
        <p style={S.p}><b>Stock first at the split.</b> A <em>plated</em> line (EP, MEP, P25) is a stocked finished good, not plating work: after the sales order posts, the split checks live stock. In stock → a WMS pick line. Short → a <b>true backorder</b> (below). Unreadable → picked with a red "stock unknown" warning, never invented as a shortage. An order with nothing to spray gets a <b>pick-only</b> document: the WMS picks and packs it, the finishing floor never sees it.</p>
        <p style={S.p}><b>True backorders.</b> A line is a true backorder when nothing on the shelf can make it — plated: none on hand; painted: no finished stock, no /P and no raw mill core. The sales-order card shows <b>⛔ BACKORDER · n lines</b> (the lines in the hover). Work them on <b>12.5 Stock View → 📋 Backorders</b>: oldest order first, each line with what covers it (open PO / WO), UNCOVERED in red when nobody has ordered it, <em>Order it</em> drops the shortfall into the grid's Order column.</p>
        <p style={S.p}><b>Order Entry sales orders are on the board too</b> (grey "Order Entry · stocked — record only" group under Sales Orders): the warehouse picks and packs those straight off the shelf, so the card is a record with a WMS chip and its linked work orders — never a split button.</p>
        ...
        <p style={S.p}><b>Rod cuts.</b> A 4 ft or 6 ft pole order raises its cut (or picks the length straight off the shelf when it is there). The open cuts are listed read-only under Work Orders on RTG — <b>✂ Rod cuts open</b> — as the file cabinet; the cut is DONE only at WMS → Rod Cuts. Cancelling a cut there lifts the order's gate; the Setup Queue refuses to start an order whose cut is still open.</p>
        <p style={S.p}><b>Purchase orders.</b> The Purchase Orders panel shows <b>every open PO</b> for the brand — Draft, Approved, queued, pushed, sent, partially received — with its lines, where each came from, what has arrived and the running total. <b>✓ Approve → NetSuite</b> is the one deliberate act (it gets the PO #); <b>✉ Mark sent to vendor</b> records the send.</p>
        <p style={S.p}><b>Close and Delete mean the same everywhere.</b> Closing or deleting a record on RTG closes its floor docs, cancels its open rod cut, cancels any NetSuite write still queued for it, and raises the NetSuite close task; a write already in flight is flagged so a person can close the transaction there. The closer keeps the state it replaced on each document, so a later reopen restores it exactly. <b>"Finished on the floor, still live here"</b> is raised only when every floor document of the order is packed or built and the record does not already carry the floor's report — a finishing job that is Complete but not packed is still in the warehouse's hands, and a pick-only order is never finished until it is packed. That group has no Close all: close such an order from its own row, or let put-away and shipment close it. Scrap reported at QC reaches the RTG row in red. The <b>Board vs Floor</b> panel finds what is left: orders held at a gate nothing can lift (<b>⬆ Lift the gate</b> stops the wait without releasing), floor jobs with no record, NetSuite transactions posted after their order closed.</p>
        <p style={S.p}><b>Reopening a bulk close.</b> If a <b>⇄ Close all</b> on the Board vs Floor panel closed orders that were still being worked, <b>⟲ Reopen a bulk close</b> (in the panel's header) reads every document that day's bulk close stamped and shows a dry run first: each finishing job, shop job, record, rod cut and queued NetSuite write with what will happen to it and why. Every restore comes from the document's own stamps — a fulfilment queued or posted (that is the shipment) or a put-away stays closed; packed with no fulfilment goes back to Complete with its pack; finished but not packed goes back to Complete and into the WMS pick queue with a red "reopened — confirm pick state" flag; part-way through its tasks goes back to Painting; not started goes back to Setup. A record reopens only when one of its floor documents is still work. Nothing is written until you press <b>⟲ Reopen n now</b>; every write is stamped with who and when and lands in the ledger.</p>
        <p style={S.p}><b>A refused NetSuite queue is not invisible.</b> When a CPQ save's NetSuite estimate or sales order is refused (a customer with no NetSuite id, a line with no library item or no NetSuite internal id), the quote is still saved and the refusal is stamped on it. RTG's <b>⇄ Quotes &amp; Sales Orders</b> panel lists it in red as <b>REFUSED</b> with the code and the message, and <b>⇄ Queue now</b> runs the same build again against the library as it stands — fix the item or the customer first, then press it. A success queues the transaction and clears the refusal; another refusal updates the reason and sends nothing.</p>
        <p style={S.p}><b>Reopening one order.</b> Beside <b>⟲ Reopen a bulk close</b> the Board vs Floor header takes an order — its SO number, its WO id or its NetSuite work-order number — and <b>⟲ Reopen one</b> shows the same dry run for that order alone, from whatever screen closed it, restored from the state the closer kept. A "closed here, still live on the floor" row offers the same button. The shop cannot undo a close RTG made; this is where that is done.</p>
        <p style={S.p}><b>The transmit panel says FAILED when NetSuite refused a write.</b> A queued quote or sales order whose staged write failed reads FAILED with NetSuite's error instead of "posting…"; fix the cause (a missing Class, an item with no NetSuite id) and retry it in 11.1 → NetSuite Sync Queue.</p>
        <p style={S.p}><b>A purchase order's lines are final once it has left us.</b> The ✎ edit on an RTG purchase-order card is offered only while the PO is a draft; once it is queued, numbered, sent or acknowledged the lines lock and the editor refuses with the reason. Receiving still works; anything further is a new PO.</p>
        <p style={S.p}><b>Reading a Setup Queue card: picked is not started.</b> On an order with a custom half, the small-parts pick is released when the <em>shop</em> presses Start, so "Warehouse · picked, at staging" can sit above "Finishing · setup not started" for days: the parts are staged, the paint run has not begun. <b>Start Setup</b> is the floor's own step. Two neighbours showing <em>Stage to Floor</em> instead simply had it pressed. Set the group's run day on the header so the rest of the team can plan around the batch.</p>
        <p style={S.p}><b>Where a stuck order says why.</b> The chip on RTG; the Setup Queue card — a PENDING-RECIPE order names where the recipe was looked for and its sales order, and "not sent to WMS" says why; the WMS pending window; <em>Where is it?</em> on every screen. A traverse order carries its fascia / track / F-clip cuts by drive to both floors.</p>
...
const ShopFloorGuide = () => (
    <div>
        ...
        <Screen title="Staged → Active → Complete & Label" tag="one card, one job, every tablet in step">
            ...
            <Path name="Traverse job" goes="cut exactly what each row says">A traverse card shows a <b>Traverse Cut Sheet</b> instead of the pole sheet: the drive (manual or motorized) and the setup in the header, then one row per part — <b>Fascia</b>, <b>Track</b>, <b>F-clip</b> — with its cut length and how many. The lengths already have the drive's deduction taken off by Vision; never re-derive them from the fascia length at the saw.</Path>
            <Path name="No cut sheet from Vision" goes="red banner — confirm the numbers with HQ before cutting">A card with a cut-to but no O2O, bends or cut sheet, on an order that came through Vision, was drawn before it was engineered. The banner says so on the staged row and the active card. A plain straight cut-to-length pole never shows it — the cut-to is the whole sheet for those.</Path>
            <Path name="Several configurations">When one order carries several configurations (each with its own checkbox), the cut sheet shown is the <b>first</b> configuration's. Open each row's 🔍 View Item for the others before cutting.</Path>
