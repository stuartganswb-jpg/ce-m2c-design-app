   ...
   useEffect(() => {
    ...
    const pushWOsToDispatch = async (opts = {}) => {
        ...
        try {
            ...
            // ── THE ONE WRITER (Brief A, A1 step 2 — 2026-09-02). The same call the Sales Snapshot
            // makes: identity, route, the COMPLETE pre-built finishing doc, the pre-check gates, the
            // rod cut and the anchor policy all come from Shared/workOrderCreate — so a grid order
            ...
            for (let key = 0; key < prepped.length; key++) {
                ...
                const erp = String(erpId).toUpperCase();
                if (routeForCode(erp).refuse === REFUSE_PHOSPHATE) {
                    const baseErp = millBaseOf(erp);
                    const basePart = hqParts.find(p => String(p.legacyErpId || p.itemId || '').toUpperCase() === baseErp) || null;
                    await createConvertDemands([{
                        r: { itemid: erp, internalId: part.netSuiteInternalId ? String(part.netSuiteInternalId) : null },
                        info: { part, available: nsStock[erp]?.available ?? 0, minOnHand: part.manufacturingSpecs?.reorderPoint ?? 0 },
                        qty: Number(qty), baseErp,
                        baseInfo: { part: basePart, iid: basePart?.netSuiteInternalId ? String(basePart.netSuiteInternalId) : null, available: nsStock[baseErp]?.available ?? null },
                    }]);
                    madeUp.push(`⇄ ${erp}: Convert to-do ${baseErp} → ${erp} ×${qty} (WMS Convert tab) — a /P is phosphated, never work-ordered`);
                    continue;
                }
                try {
                    const res = await parkWorkOrder({
                        ...
                        note: `Stock View grid · ${erp} ×${qty}${pre && pre.plan && pre.plan.exploded ? ` · BOM pull: ${pre.plan.lines.map(l => `${l.quantity}×${l.legacyErpId}`).join(', ')}` : ''}`,
                        source: 'STOCKVIEW_GRID', precheck: pre, pins, inventory: hqParts,
                        locationId: (BRAND_NETSUITE_MAP[activeBrand] || {}).location || '17',
                    });
                    ...
                    // A refusal is the writer saying this row is not a work order: nothing was
                    // written, say so, next row.
                    if (e instanceof ParkRefusal) {
                        addLog(`⛔ ${erp}: ${e.message}`, 'error');
                        madeUp.push(`⛔ ${erp}: SKIPPED — ${e.message}`);
                        continue;
                    }
                    throw e;
                }
            ...
            addLog(`✅ ${n} work order(s) parked in RTG Dispatch — routed, gated, released from there on their own.`, "success");
            alert(`✅ ${n} work order(s) parked in RTG Dispatch.\n\nEach carries its route and its complete floor job; RTG releases it on its own once its gates are clear.${madeUp.length ? `\n\nWhat was raised:\n${madeUp.map(m => `• ${m}`).join('\n')}` : ''}`);
            setOrderDrafts({});
        } catch(e) {
    ...
    const openSalesHistory = async (forceRebuild = false) => {
        ...
        try {
            ...
            // runChunked (Shared/stockPosition) splits any chunk that answers at the 1000-row cap
            // and keeps 4 in flight — the why is written there. A chunk size means "how many ids
            // can I ask about before the ANSWER might exceed 1000 rows": exactly one row per id
            // for availability, one per month in range for sales, one per open line for PO/WO.
            //
    ...
    // wait, and a failure leaves the column blank rather than the report unopenable. The tally
    // itself is the ONE reader (Shared/stockPosition.backorderTallyOf) the 4.7 board reads too.
    const loadBackorderTally = async () => {
        try {
            const snap = await getDocs(query(collection(db, 'hq_sales_orders'), where('brand', '==', activeBrand)));
            setBoByCode(backorderTallyOf(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
        } catch (e) { setBoByCode({}); }
    };
            ...
            <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1.5, background: '#fff', border: '1px solid var(--line)', display: 'flex', flexDirection: 'column', borderRadius: '2px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
                    <div style={{ padding: '24px', background: 'var(--paper-2)', borderBottom: '1px solid var(--line)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {/* ⚠ MUST pass the argument explicitly. `onClick={openSalesHistory}` handed React's
                            click event in as `forceRebuild` — and a MouseEvent is truthy, so every open
                            skipped the cache read and re-derived all 12 months from NetSuite, then rewrote
                            11 cache docs it never read. The month caching below worked perfectly and was
                            simply never asked a question (Stuart 2026-09-04: "it is almost like the cached
                            sales data made no difference"). It made none. */}
                        <button onClick={() => openSalesHistory(false)} style={{ marginLeft: 'auto', padding: '8px 14px', background: 'var(--brass)', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '.1em' }}>📈 Stocked Sales Snapshot</button>
