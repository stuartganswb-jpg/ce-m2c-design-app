                                        ...
                                        {r.choices.map((c) => (
                                            <React.Fragment key={c.nodeName}>
                                                ...
                                                <span style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                                    ...
                                                    <select value={(c.custIds && c.custIds[0]) || ''} title="CUSTOMER-ONLY choice: pick a customer and this option shows on the CPQ (and their portal) ONLY when that customer is selected — everyone else never sees it. '— all —' = visible to every customer (poles, rings, common parts)." onFocus={() => ensureCustomers()} onChange={e => { const id = e.target.value; const nm = ((custList || []).find(x => x.id === id) || {}).name || ''; setChoicePatch(r.clusterId, c.nodeName, { custIds: id ? [id] : [], custNames: id ? [nm] : [] }); }} style={{ ...inp, padding: '1px 2px', fontSize: '7.5px', fontFamily: 'var(--mono)', width: '82px', maxWidth: '82px', borderColor: (c.custIds && c.custIds.length) ? 'var(--brass)' : 'var(--line)', color: (c.custIds && c.custIds.length) ? 'var(--brass)' : 'var(--ink-soft)' }}>
