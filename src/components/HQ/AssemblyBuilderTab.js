                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: normalizeCategory(r.category) === 'FINIAL' ? '48px 1fr 210px 122px 52px 46px' : '48px 1fr 210px 52px 46px', gap: '6px 10px', alignItems: 'center' }}>
                                        {r.choices.map((c) => (
                                            <React.Fragment key={c.nodeName}>
                                                ...
                                                <input value={c.itemNo} list="ab-item-codes" onChange={e => setChoicePatch(r.clusterId, c.nodeName, { itemNo: e.target.value })} placeholder={c.isFee ? 'fee — item # optional (links the fee entity, e.g. CE-FEE-4594, for pricing)' : (c.isHidden ? 'hidden — item # optional (adds it to the BOM)' : 'item # — type to search (blank = hardware)')} style={{ ...inp, padding: '5px 8px', fontSize: '0.78rem', fontFamily: 'var(--mono)', borderColor: c.isFee ? 'var(--line)' : (c.itemNo ? 'var(--brass)' : 'var(--line)'), opacity: c.isFee ? 0.5 : 1 }} />
                                                {!(c.itemNo && c.itemNo.trim()) && !c.isFee && !c.isHidden && looksRealPart(c.label) && (
                                                    <span title="No item # yet — on save this choice PARKS: the node hides from the model AND the flow until you assign the # (Load Choices keeps it listed here). Perfect for parts IT hasn't set up yet." style={{ fontFamily: 'var(--mono)', fontSize: '8px', color: 'var(--brass)', border: '1px solid var(--brass)', padding: '2px 6px', whiteSpace: 'nowrap' }}>⏸ parks</span>
                                                )}
                                                ...
                                                <span style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                                    <label title="Fee choice (e.g. a french-return bend): shows this geometry as a selectable option, bills as a fee — no item # / BOM line. Position comes from the cluster's tag." style={{ display: 'flex', alignItems: 'center', gap: '4px', fontFamily: 'var(--mono)', fontSize: '8px', letterSpacing: '.05em', textTransform: 'uppercase', color: c.isFee ? 'var(--brass)' : 'var(--ink-soft)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                                        <input type="checkbox" checked={!!c.isFee} onChange={e => setChoicePatch(r.clusterId, c.nodeName, { isFee: e.target.checked, ...(e.target.checked ? { isHidden: false, isBasic: false, usesReturnPlates: false } : {}) })} style={{ cursor: 'pointer' }} />
                                                        fee
                                                    </label>
                                                    ...
                                                    <select value={(c.custIds && c.custIds[0]) || ''} title="CUSTOMER-ONLY choice: pick a customer and this option shows on the CPQ (and their portal) ONLY when that customer is selected — everyone else never sees it. '— all —' = visible to every customer (poles, rings, common parts)." onFocus={() => ensureCustomers()} onChange={e => { const id = e.target.value; const nm = ((custList || []).find(x => x.id === id) || {}).name || ''; setChoicePatch(r.clusterId, c.nodeName, { custIds: id ? [id] : [], custNames: id ? [nm] : [] }); }} style={{ ...inp, padding: '2px 3px', fontSize: '8px', fontFamily: 'var(--mono)', width: '104px', borderColor: (c.custIds && c.custIds.length) ? 'var(--brass)' : 'var(--line)', color: (c.custIds && c.custIds.length) ? 'var(--brass)' : 'var(--ink-soft)' }}>
                                                    ...
                                                    <label title="INLINE BRACKET (the bracket-side sibling of the basic tag): while this In Line arm is the selected bracket, the plate picker shows the INLINE plates (inl-only copies) — or the RETURN plates when the flow has no inline copies. This tag is how the system knows a bracket is In Line." style={{ display: 'flex', alignItems: 'center', gap: '4px', fontFamily: 'var(--mono)', fontSize: '8px', letterSpacing: '.05em', textTransform: 'uppercase', color: c.usesReturnPlates ? 'var(--brass)' : 'var(--ink-soft)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                                    ...
                                                    {normalizeCategory(r.category) === 'BACKPLATE' && (
                                                        <label title="RETURN BACKPLATE: offered ONLY while this end is a french/miter return or an end-return arm — the regular plates hide then (and this one hides otherwise). Use for the plates that pair with the return/arm." style={{ display: 'flex', alignItems: 'center', gap: '4px', fontFamily: 'var(--mono)', fontSize: '8px', letterSpacing: '.05em', textTransform: 'uppercase', color: c.returnOnly ? 'var(--brass)' : 'var(--ink-soft)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                                            <input type="checkbox" checked={!!c.returnOnly} onChange={e => setChoicePatch(r.clusterId, c.nodeName, { returnOnly: e.target.checked })} style={{ cursor: 'pointer' }} />
                                                            rtn-only
                                                        </label>
                                                    )}
                                                    {normalizeCategory(r.category) === 'BACKPLATE' && (
                                                ...
                                                <React.Fragment key={c.nodeName}>
                                                    ...
                                                    <span style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
                                                        <label title="Fee choice (e.g. a french-return bend): selectable option that bills as a fee — no item # / BOM line." style={{ display: 'flex', alignItems: 'center', gap: '3px', fontFamily: 'var(--mono)', fontSize: '8px', textTransform: 'uppercase', color: c.isFee ? 'var(--brass)' : 'var(--ink-soft)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                                            <input type="checkbox" checked={!!c.isFee} onChange={e => setSlotChoicePatch(slot.id, c.nodeName, { isFee: e.target.checked, ...(e.target.checked ? { isHidden: false, isBasic: false, usesReturnPlates: false } : {}) })} style={{ cursor: 'pointer' }} />fee
                                                        </label>
                                                        ...
                                                        {normalizeCategory(slot.category) === 'BACKPLATE' && (
                                                            <label title="INLINE BACKPLATE: inline-bracket copy of a shared return-style plate — offered only while an inl-bkt bracket is selected." style={{ display: 'flex', alignItems: 'center', gap: '3px', fontFamily: 'var(--mono)', fontSize: '8px', textTransform: 'uppercase', color: c.inlineOnly ? 'var(--brass)' : 'var(--ink-soft)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
