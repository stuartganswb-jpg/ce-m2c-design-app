                                        ...
                                        {r.choices.map((c) => (
                                            <React.Fragment key={c.nodeName}>
                                                ...
                                                <span style={{ display: 'flex', flexWrap: 'wrap', gap: '7px 18px', alignItems: 'center', minWidth: 0 }}>
                                                    ...
                                                    <label title="TAKES NO FINISH — this part is clear/acrylic and never takes the selected finish. Its metal COLLAR still does (tag the collar separately as a collar, not as this). Replaces the hardcoded acrylic item-code list in the renderer: the new engine reads this tag and nothing else, so a new acrylic finial is a tick here rather than a code change." style={{ display: 'flex', alignItems: 'center', gap: '6px', fontFamily: 'var(--mono)', fontSize: '10px', letterSpacing: '.05em', textTransform: 'uppercase', color: c.noFinish ? 'var(--brass)' : 'var(--ink-soft)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                                                        <input type="checkbox" checked={!!c.noFinish} onChange={e => setChoicePatch(r.clusterId, c.nodeName, { noFinish: e.target.checked })} style={{ cursor: 'pointer', width: '15px', height: '15px', margin: 0, flexShrink: 0 }} />
                                                        no-finish
                                                    </label>
