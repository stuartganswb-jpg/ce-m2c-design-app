...
import { TRAVERSE_ROLES, DRIVE_TYPES, TRV_SETUPS, FRONT_LAYERS, FRONT_LAYER_LABEL, suggestSetupFromName } from '../Shared/traverseTags';
...
function ChoiceTagControls({ c, category, onPatch, dictLists, custList, ensureCustomers, collarCands, inp }) {
    return (
        <span style={{ display: 'flex', flexWrap: 'wrap', gap: '7px 18px', alignItems: 'center', minWidth: 0 }}>
            ...
            {/* RIDES WITH (Stuart 2026-09-10): a hidden part that comes only when a return is on the
            ...
            {/* WHICH FRONT OF A DOUBLE (Stuart 2026-09-06, H1-2TRV): "when you choose Stationary fascia
                with rings that should present the only bracket option of H1-2TRV-DRTWB and if we
                choose Traverse track front & rear then present H1-2TRV-DWB — the double brackets are
                one or the other, not interchangeable." Brackets only; blank = either, which is what
                every bracket that is not a double genuinely is. Read by admits() beside drive. */}
            {normalizeCategory(c.catOverride || category) === 'BRACKET' && !c.isFee && (
                <select value={c.frontLayer || ''} title="FRONT OF A DOUBLE — which front this bracket carries: a stationary FASCIA with rings (one track behind it) or a TRACK front & rear. Only filters on a DOUBLE once the Front-of-the-Double question is answered. Blank = either." onChange={e => onPatch({ frontLayer: e.target.value })} style={{ ...inp, padding: '3px 5px', fontSize: '9px', fontFamily: 'var(--mono)', width: '124px', maxWidth: '124px', borderColor: c.frontLayer ? 'var(--brass)' : 'var(--line)', color: c.frontLayer ? 'var(--brass)' : 'var(--ink-soft)' }}>
                    <option value="">front: — either —</option>
                    {FRONT_LAYERS.map(t => <option key={t} value={t}>front: {FRONT_LAYER_LABEL[t]}</option>)}
                </select>
            )}
            ...
            {/* ⚠ IT SHOWS ON RETURNS TOO, NOT ONLY ON BRACKET CLUSTERS (Stuart 2026-09-06, H1-2TRV:
                "i do not see where the end arm tag is").
                The tag was born as the Flat Iron pattern — a bracket that IS the end — so the box
                only ever drew on a BRACKET cluster. H1-2TRV is the mirror image: a RETURN, living
                in a finial cluster, that needs a bracket under it because it holds nothing up. Same
                flag, opposite direction, and the pair it forms with NO PLATE was impossible to set
                because half of it was never on screen.
                Returns only — a plain finial or an end cap has no use for it, and an extra tick on
                every end row is how a tag gets set by accident. */}
            {(normalizeCategory(c.catOverride || category) === 'BRACKET'
              || ['FRENCH_RETURN', 'MITER_RETURN'].includes(String(c.endTreatment || '').toUpperCase())) && (
                <label title={normalizeCategory(c.catOverride || category) === 'BRACKET'
                    ? "END RETURN ARM (Flat Iron pattern): this bracket IS the end treatment — selecting it greys that side's finial / inside-mount step; its backplate stays choosable (unless basic). Auto-detected when the library part is flagged isReturnBracket; check here to force it."
                    : "END RETURN ARM on a RETURN: this end arm is DECORATIVE — it clamps on and holds nothing up, so the rod still needs a real bracket at that end. Tick it TOGETHER WITH NO PLATE and the left/right bracket step stays open instead of being replaced. Either tag on its own keeps today's behaviour: a return with no plate still replaces the bracket."} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontFamily: 'var(--mono)', fontSize: '10px', letterSpacing: '.05em', textTransform: 'uppercase', color: c.isReturnArm ? 'var(--brass)' : 'var(--ink-soft)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            ...
            mergeSlots.forEach((slot, slotIdx) => {
                ...
                (layer.choices || []).forEach((ch, idx) => {
                    ...
                    pins.push({
                        ...
                        ...(ch.isHidden ? { isHiddenPart: true } : {}), ...(ch.isHidden && String(ch.ridesWith || '').trim() ? { ridesWith: String(ch.ridesWith).trim().toUpperCase() } : {}),
                        ...(ch.isBasic && !ch.isFee && !ch.isHidden ? { isBasic: true } : {}), ...(ch.noBackplate ? { noBackplate: true } : {}), ...(ch.usesReturnPlates && !ch.isFee && !ch.isHidden ? { usesReturnPlates: true } : {}), /* ⚠ A RETURN IS A FEE HERE, so the old `!ch.isFee` guard threw this tag away on save
                       (Stuart 2026-09-06, H1-2TRV). Picking MITER/FRENCH RETURN in the End Treatment
                       select sets isFee automatically, so ticking end-arm on a return appeared to work
                       and vanished on the next write — while NO PLATE, its partner, has never had the
                       guard. A hidden pin is still never a question, so that half stays. */
                       ...(ch.isReturnArm && !ch.isHidden ? { isReturnArm: true } : {}), ...(ch.returnOnly && !ch.isFee && !ch.isHidden ? { returnOnly: true } : {}), ...(ch.inlineOnly && !ch.isFee && !ch.isHidden ? { inlineOnly: true } : {}),
        ...
        try {
            ...
            if (!cadUrl && data.manufacturingSpecs?.sheet2d?.url) {
                ...
                const rows2 = clusters.filter(c => c.region2d && !c.hidden).map(cl => {
                    ...
                    const choices = pins2.map(pin => {
                        ...
                        return { nodeName: pin.choiceNode, label: pin.partName || '', itemNo, imgUrl: pin.imageUrl || '', endTreatment: '', isFee: !!pin.isFee, isHidden: !!pin.isHiddenPart && !pin.parked, parked: !!pin.parked, ridesWith: pin.ridesWith || '', isBasic: false, usesReturnPlates: false, isReturnArm: false, returnOnly: false, inlineOnly: false, isCollar: false, requiresCollar: '', projInches: '', mountType: '', catOverride: '', custIds: pin.customerIds || [], custNames: pin.customerNames || [], traverseRole: '', driveType: '', trvSetup: '', frontLayer: '', tier: '', alwaysShown: !!pin.alwaysShown, note: pin.designerNote || '', thumb: '' };
                    });
            ...
            const rows = clusters.map(cl => {
                ...
                const choices = kids.map(nm => {
                    ...
                    return { nodeName: nm, label, itemNo, endTreatment: et, isFee: !!pin?.isFee, isHidden: !!pin?.isHiddenPart && !pin?.parked, parked: !!pin?.parked, ridesWith: pin?.ridesWith || '', isBasic: !!pin?.isBasic, noBackplate: !!pin?.noBackplate, usesReturnPlates: !!(pin?.usesReturnPlates || cl.usesReturnPlates), isReturnArm: !!(pin?.isReturnArm || cl.isReturnArm), returnOnly: !!(pin?.returnOnly || cl.returnOnly), inlineOnly: !!(pin?.inlineOnly || cl.inlineOnly), isCollar: !!pin?.isCollar, requiresCollar: pin?.requiresCollar || '', projInches: pin?.projInches || '', mountType: pin?.mountType || '', catOverride: pin?.catOverride || '', custIds: pin?.customerIds || [], custNames: pin?.customerNames || [], traverseRole: pin?.traverseRole || '', driveType: pin?.driveType || '', trvSetup: pin?.trvSetup || suggestSetupFromName(label, itemNo, nm), frontLayer: pin?.frontLayer || '', tier: pin?.tier || '', passing: pin?.passing || '', alwaysShown: !!pin?.alwaysShown, noFinish: !!pin?.noFinish, materials: pin?.materials || '', note: pin?.designerNote || '', thumb: '' };
                });
        ...
        try {
            ...
            for (const r of assignData.rows) {
                for (let idx = 0; idx < r.choices.length; idx++) {
                    ...
                    qSet(doc(db, 'assembly_pins', pid), { id: pid, assemblyId: assignData.asmId, clusterId: r.clusterId, partId, partName: ch.label || partId, defaultQty: 1, choiceNode: ch.nodeName, targetNode: ch.nodeName, choiceSort: idx, ...(ch.endTreatment && !(ch.catOverride && String(ch.catOverride).toUpperCase() !== 'FINIAL') ? { endTreatment: ch.endTreatment } : {}), ...(ch.isFee && !ch.isHidden ? { isFee: true } : {}), ...(ch.isHidden ? { isHiddenPart: true } : {}), ...(ch.isHidden && String(ch.ridesWith || '').trim() ? { ridesWith: String(ch.ridesWith).trim().toUpperCase() } : {}), ...(ch.isBasic && !ch.isFee && !ch.isHidden ? { isBasic: true } : {}), ...(ch.noBackplate ? { noBackplate: true } : {}), ...(ch.usesReturnPlates && !ch.isFee && !ch.isHidden ? { usesReturnPlates: true } : {}), ...(ch.isReturnArm && !ch.isHidden ? { isReturnArm: true } : {}), ...(ch.returnOnly && !ch.isFee && !ch.isHidden ? { returnOnly: true } : {}), ...(ch.inlineOnly && !ch.isFee && !ch.isHidden ? { inlineOnly: true } : {}), ...(hasItem && !ch.isFee && Array.isArray(ch.custIds) && ch.custIds.length ? { customerIds: ch.custIds, customerNames: ch.custNames || [] } : {}), ...(hasItem && !ch.isFee && ch.isCollar ? { isCollar: true } : {}), ...(hasItem && !ch.isFee && !ch.isCollar && String(ch.requiresCollar || '').trim() ? { requiresCollar: String(ch.requiresCollar).trim() } : {}), ...(hasItem && String(ch.projInches || '').trim() ? { projInches: String(ch.projInches).trim().toUpperCase() } : {}), ...(hasItem && !ch.isFee && String(ch.mountType || '').trim() ? { mountType: String(ch.mountType).trim().toUpperCase() } : {}), ...(hasItem && String(ch.catOverride || '').trim() ? { catOverride: String(ch.catOverride).trim().toUpperCase() } : {}), ...(String(ch.traverseRole || '').trim() ? { traverseRole: String(ch.traverseRole).trim().toUpperCase() } : {}), ...(String(ch.driveType || '').trim() ? { driveType: String(ch.driveType).trim().toUpperCase() } : {}), ...(String(ch.trvSetup || '').trim() ? { trvSetup: String(ch.trvSetup).trim().toUpperCase() } : {}), ...(String(ch.frontLayer || '').trim() ? { frontLayer: String(ch.frontLayer).trim().toUpperCase() } : {}), ...(String(ch.tier || '').trim() ? { tier: String(ch.tier).trim().toUpperCase() } : {}), ...(String(ch.passing || '').trim() ? { passing: String(ch.passing).trim().toUpperCase() } : {}), ...(ch.alwaysShown && !ch.isFee ? { alwaysShown: true } : {}), ...(ch.noFinish ? { noFinish: true } : {}), ...(String(ch.materials || '').trim() ? { materials: String(ch.materials).trim().toUpperCase() } : {}), // THE FEE'S TYPED CODE, IN A FIELD OF ITS OWN (Stuart 2026-08-04: "the bug that keeps
                    // whipping out all my miter fees is still, they are yet again gone"). partId was
                    // carrying it, and partId is rewritten by anything that re-saves a choice — once it
                    // became the synthetic FEE-<slug> the code was unrecoverable. This field is written
                    // by nothing else and read back first, so the code survives a bad round-trip.
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: normalizeCategory(slot.category) === 'FINIAL' ? '40px minmax(130px,200px) 180px 110px minmax(280px,1fr) 40px' : '40px minmax(130px,200px) 180px minmax(280px,1fr) 40px', gap: '5px 10px', alignItems: 'center' }}>
                                            {(layer.choices || []).map(c => (
                                                ...
                                                {(() => {
                                                    ...
                                                    <ChoiceTagControls c={c} category={slot.category} onPatch={p => setSlotChoicePatch(slot.id, c.nodeName, p)} dictLists={dictLists} custList={custList} ensureCustomers={ensureCustomers} collarCands={slotCollarCands} inp={inp} />
                                                    <span style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
