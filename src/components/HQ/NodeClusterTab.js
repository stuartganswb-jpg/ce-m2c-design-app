                                            ...
                                            <div style={{ minWidth: 0 }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '5px', marginTop: '5px' }}>
                                                    ...
                                                    {String(cl.category || '').toUpperCase() === 'BACKPLATE' && (
                                                        <button onClick={() => setClusterField(cl.id, { returnOnly: !cl.returnOnly })} title="RETURN plates: offered ONLY while that end is a french/miter return or an end-return arm — the regular plates hide then (and these hide otherwise). Regenerate the flow after changing." style={{ padding: '3px 8px', background: cl.returnOnly ? 'var(--brass)' : '#fff', color: cl.returnOnly ? '#fff' : 'var(--ink-soft)', border: `1px solid ${cl.returnOnly ? 'var(--brass)' : 'var(--line)'}`, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '.05em' }}>{cl.returnOnly ? '✓ RETURN plates' : '⊘ Return plates'}</button>
                                                    )}
                                                    {String(cl.category || '').toUpperCase() === 'BACKPLATE' && (
                                                        <button onClick={() => setClusterField(cl.id, { inlineOnly: !cl.inlineOnly })} title="INLINE plates: the INLINE-bracket copies of shared return-style plates — offered ONLY while a rtn-bp (In Line) bracket is selected; the RETURN-plates copies show for actual returns. Regenerate after changing." style={{ padding: '3px 8px', background: cl.inlineOnly ? 'var(--brass)' : '#fff', color: cl.inlineOnly ? '#fff' : 'var(--ink-soft)', border: `1px solid ${cl.inlineOnly ? 'var(--brass)' : 'var(--line)'}`, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '.05em' }}>{cl.inlineOnly ? '✓ INLINE plates' : '⊘ Inline plates'}</button>
                                                    )}
                                                    {String(cl.category || '').toUpperCase() === 'BRACKET' && (
                                                        <button onClick={() => setClusterField(cl.id, { isReturnArm: !cl.isReturnArm })} title="END RETURN ARM (Flat Iron pattern): this bracket IS the end treatment — selecting it greys that side's finial / inside-mount step and flips the plate picker to the RETURN plates. Regenerate the flow after changing." style={{ padding: '3px 8px', background: cl.isReturnArm ? 'var(--brass)' : '#fff', color: cl.isReturnArm ? '#fff' : 'var(--ink-soft)', border: `1px solid ${cl.isReturnArm ? 'var(--brass)' : 'var(--line)'}`, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '.05em' }}>{cl.isReturnArm ? '✓ END return arm' : '⊘ End return arm'}</button>
                                                    )}
                                                    {String(cl.category || '').toUpperCase() === 'BRACKET' && (
                                                        <button onClick={() => setClusterField(cl.id, { usesReturnPlates: !cl.usesReturnPlates })} title="INLINE bracket (In Line arm) — the bracket-side sibling of the basic tag: while selected, the plate picker shows the INLINE plates (or the RETURN plates when the flow has no inline copies). Regenerate after changing." style={{ padding: '3px 8px', background: cl.usesReturnPlates ? 'var(--brass)' : '#fff', color: cl.usesReturnPlates ? '#fff' : 'var(--ink-soft)', border: `1px solid ${cl.usesReturnPlates ? 'var(--brass)' : 'var(--line)'}`, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '.05em' }}>{cl.usesReturnPlates ? '✓ INLINE bracket' : '⊘ Inline bracket'}</button>
                                                    )}
                                                </div>
