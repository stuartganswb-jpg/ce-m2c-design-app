...
// (the auto-release ENGINE effect lives below the live-mirror subscriptions — its dependency
    ...
    // MOVED to Shared/floorRelease.js (Stuart 2026-09-04: a release must never depend on the RTG
    // tab being open) — the WMS rod-cut / convert completions and A's clearConvertGate call the same
    // function. This wrapper keeps RTG's three callers and their addLog wiring unchanged.
    const queueNsStockWorkOrder = (hqOrder, fp) => queueNsStockWorkOrderShared({ hqOrder, fp, brand: activeBrand, by: currentUser || '', log: addLog });
    ...
    const pushToFinishing = async (hqOrder, orderType, opts = {}) => {
        ...
        if (hqOrder.finPayload && hqOrder.finPayload.id) {
            try {
                ...
                // ONE shared release (Shared/floorRelease.releaseStockWoToFloor): the verbatim
        ...
        return (
            <div style={{ padding: '12px 24px', borderBottom: '1px solid var(--line)', background: '#fffdf7' }}>
                ...
                <div style={{ fontSize: '11px', color: 'var(--ink-soft)', margin: '4px 0 8px', lineHeight: 1.5 }}>
                    Every document today's "Close all" stamped, decided from its OWN stamps: fulfilment queued or posted (= shipped) or put away → kept closed; packed with no fulfilment → Complete with its pack; finished on the floor but not packed → Complete and back in the WMS pick queue, flagged "reopened — confirm pick state"; part-way through its tasks → Painting; nothing started → Setup. A shop half comes back to In Process / Sent to Plating / Completed from its own stamps (one reopened by hand keeps its status and loses the hidden closed flag). A record reopens only when one of its floor documents is still work. A NetSuite write the close cancelled goes back to PENDING, or to FAILED if it had already failed (retry by hand in 11.1). Nothing is written until you press Reopen.
