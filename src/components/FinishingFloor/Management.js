...
const Management = ({ sysConfig, users, logs, writeLog, user, perms, setPerms }) => {
    ...
    const [config, setConfig] = useState({
        ...
        spinMins: sysConfig?.spinMins || 6,
        ovenMins: sysConfig?.ovenMins || 6,
    ...
    if (user?.role !== 'admin' && user?.role !== 'floor_manager') {
        return <div style={{ padding: '30px' }}><h2>ACCESS DENIED</h2><p>You need Admin or Floor Manager privileges to view this page.</p></div>;
    }
    ...
    const handleSaveUser = async () => {
        ...
        if (oldId && oldId !== uPin) {
            await deleteDoc(doc(db, "fin_users", oldId));
        }
    ...
    const togglePermission = (tab, role) => {
        ...
        if (newPerms[role].includes(tab)) {
            newPerms[role] = newPerms[role].filter(t => t !== tab);
        } else {
            newPerms[role] = [...newPerms[role], tab];
        }
        setPermissions(newPerms);
    };

    // --- DATABASE WIPE & SYNC FUNCTIONS ---
    ...
    // NEW: Smart Legacy Sync for Paint Profiles
    const syncLegacyPaintProfiles = async () => {
        ...
        try {
            ...
            // Check fin_paint_profiles first, if empty fallback to paint_profiles
            let snap = await oldGetDocs(oldCol(oldDb, "fin_paint_profiles"));
            if (snap.empty) {
                snap = await oldGetDocs(oldCol(oldDb, "paint_profiles"));
            }
        ...
        <div style={{ padding: '30px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
            ...
            <div>
                ...
                <div style={{ background: '#fff', border: '2px solid #333', padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                    <div><label style={labelStyle}>Mix Station Time</label><input type="number" step="0.1" value={config.mixMins} onChange={e=>setConfig({...config, mixMins: Number(e.target.value)})} style={inputStyle} /></div>
                    <div><label style={labelStyle}>Spinning Setup/Paint</label><input type="number" step="0.1" value={config.spinMins} onChange={e=>setConfig({...config, spinMins: Number(e.target.value)})} style={inputStyle} /></div>
                    ...
                    <div><label style={labelStyle}>Hand Finish (Small Part)</label><input type="number" step="0.1" value={config.handSmallMins} onChange={e=>setConfig({...config, handSmallMins: Number(e.target.value)})} style={inputStyle} /></div>
                    ...
                    <div><label style={labelStyle}>Pole Paint Time (Per Piece)</label><input type="number" step="0.1" value={config.poleMins} onChange={e=>setConfig({...config, poleMins: Number(e.target.value)})} style={inputStyle} /></div>
                    <div style={{ borderTop: '2px dashed #ccc', paddingTop: '10px' }}><label style={{...labelStyle, color: '#CC6600'}}>Pot Life (Mins)</label><input type="number" step="1" value={config.potLifeMins} onChange={e=>setConfig({...config, potLifeMins: Number(e.target.value)})} style={inputStyle} /></div>
                    <div style={{ borderTop: '2px dashed #ccc', paddingTop: '10px' }}><label style={{...labelStyle, color: '#d9534f'}}>Recoat Window (Mins)</label><input type="number" step="1" value={config.recoatMins} onChange={e=>setConfig({...config, recoatMins: Number(e.target.value)})} style={inputStyle} /></div>
                    <button onClick={handleSaveConfig} style={{ ...btnStyle, gridColumn: '1 / -1', background: '#333', color: '#fff' }}>SAVE TIMERS</button>
                </div>

                {/* DATABASE WIPE & DATA SYNC */}
                ...
                <div style={{ background: '#fff0f0', border: '2px solid #d9534f', padding: '15px' }}>
                    <p style={{ fontSize: '0.8rem', color: '#666', marginTop: 0 }}>These actions permanently delete live production data. They do NOT affect recipes, users, or inventory.</p>
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                        <button onClick={handleWipePots} style={{ flex: 1, padding: '10px', background: '#d9534f', color: '#fff', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>WIPE ALL PAINT POTS</button>
                        <button onClick={handleWipeWorkOrders} style={{ flex: 1, padding: '10px', background: '#d9534f', color: '#fff', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>WIPE ALL WORK ORDERS</button>
                    </div>
                    {/* LEGACY SYNC BUTTON */}
                    <button onClick={syncLegacyPaintProfiles} style={{ width: '100%', padding: '10px', background: '#CC6600', color: '#fff', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>🔄 SYNC LEGACY PAINT PROFILES</button>
                </div>
                ...
                <div style={{ background: '#fff', border: '2px solid #333', padding: '20px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                        <button onClick={handleSaveUser} style={{ ...btnStyle, flex: 1, background: oldId ? '#CC6600' : '#333', color: '#fff' }}>
                            {oldId ? 'UPDATE OPERATOR' : 'ADD OPERATOR'}
                        </button>
                        {oldId && (
                            <button onClick={() => { setUName(""); setUPin(""); setURole("painter"); setOldId(""); }} style={{ ...btnStyle, background: '#888', color: '#fff' }}>
                                CANCEL
                            </button>
                        )}
                    </div>
