...
const FinishingFloor = () => {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      // Sanitized name/role/superAdmin projection (no PINs) — hq_users is admin-locked.
      onSnapshot(collection(db, "directory"), (snap) => setUsers(snap.docs.map(d => ({ id: d.id, ...d.data() })))),
