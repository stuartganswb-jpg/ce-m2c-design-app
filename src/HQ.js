...
import { useNavigate } from 'react-router-dom'; // <-- Added to allow navigating back to the Hub
import './App.css';
...
function HQ() { // <-- THIS IS THE LINE THAT CHANGED
    ...
    <div className="App" style={{ minHeight: '100vh', backgroundColor: '#e5e5e5', fontFamily: 'monospace', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{ backgroundColor: activeBrand.color, color: '#fff', padding: '15px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '4px solid #000' }}>
        ...
        <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: '15px' }}>
          <span style={{ fontSize: '0.85rem' }}>OPERATOR: <strong>{userName.toUpperCase()}</strong></span>
          ...
          {/* NEW: Navigation back to Master Hub */}
...
export default HQ; // <-- THIS IS THE OTHER LINE THAT CHANGED