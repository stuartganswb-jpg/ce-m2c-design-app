...
| item | state |
|---|---|
| C1 plating hand-off | **LIVE — `9ef3331`, pushed by D in the seven-commit batch (version 1788476288789); verified in `main.697682a3.js`: demand carries `shopOrderId`, `SENT_TO_PLATING` present, old regex gone; guide paragraph in chunk 514. Until D1 ships, a plated custom order sits at 'Sent to Plating' with the pack gate shut — correct, said to Stuart.** Mirrors `CUSTOM_FAB_STATUS.SENT_TO_PLATING` / `COMPLETE`; `toPlating = isOutsourced || finishRouteOf(...).outsourced`; both local regexes deleted (acceptance grep clean); demand gains `finSiblingId / orderKey / salesOrderId / shopOrderId`. **Acceptance rows (§6 of the brief) NOT yet run live** — needs a `/EP` custom order with Stuart pinned in. Guide plating paragraph rewritten in the same push. Original plan for reference: B's proposed contract: `CUSTOM_FAB_STATUS {PENDING, IN_PROCESS, SENT_TO_PLATING:'Sent to Plating', COMPLETE}`, mirror keeps its signature, refuses unknown, stamps `customFabAt`. My change: `toPlating = order.isOutsourced === true || isOutsourcedFinishCode(finishSuffixOf(shopItemCodeOf(order)))` (both local regexes deleted), `mirrorCustomStatusToSibling(order, toPlating ? 'Sent to Plating' : 'Complete')`, demand gains `finSiblingId / orderKey / salesOrderId / shopOrderId` (D has these names as its contract). Chip wording from B: "At the plater since <date>" (`customFabAt`). |
| C2 | shipped `334c9c3` |
