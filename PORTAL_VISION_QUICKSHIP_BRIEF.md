...
**FLOW-DRIVEN rev (2026-07-25 pm, Stuart: "the vision tool must ask which brackets and end treatment and these choices come from the cpq flow" after the free-form v1 showed different numbers than the board — the math matched, the INPUTS didn't):** on product pick the page loads the flow (`portalFlow` — which now also returns `fabShape`, seeding the shape) and mirrors the internal flow-mirror: **Rod Diameter / Bracket Projection = the flow's SIZE steps** (inches resolved from the verbatim sizeMatrix registry by optId — never typed; proj options filtered per chosen dia via `projAllowedAtDia`; defaults = the family base, same as the board's "-- Default --"), **each end = that side's End Treatment step's OPTIONS** (`endStyleOf` mapping copied verbatim: FRENCH_RETURN→RETURN_BEND, MITER_RETURN→RETURN_MITER, INSIDE_MOUNT/FLUSH→FLUSH + the INSIDE mount auto-flip), **brackets = that position's Bracket & Mount step** (a chosen return locks the bracket — "— replaced by the return —", same grey rule). Free-form dia/proj inputs remain ONLY when a flow carries no SIZE steps. The picks ride as **stepId→optId `params`, validated server-side against the flow's own steps**, spread into `specs` exactly like the internal save — so staff Load-Line and CPQ Configure open PRE-PICKED on the customer's choices, and the portal readouts EQUAL the board's numbers (parity-tested on the live case: ordering 100 / ¾" / french both ends → 99.25 C2C / 100 O2O / 103 system). BFF whitelist gained 'FLUSH' as a legal endStyle.

**Polish rev (2026-07-25 eve, Stuart after first live test — "it is working"):** (1) **fee items display as DESCRIPTIONS** app-portal-wide: `customerOptName` in `functions/index.js` — a fee option's partName (the raw fee id `CE-FEE-4594` or "…(fee — set price)" authoring hints) resolves to the fee ITEM's library `itemName` (partId matched by doc id, then itemId/legacyErpId 'in' fallbacks), applied inside `sanitizeStep` so the CONFIGURATOR display is fixed too; unresolvable ids at worst get the "(fee…)" suffix stripped. (2) **Static fit diagrams** on the "Will it fit?" card (`FitDiagram` in VisionIntake.jsx): per-shape SVGs in the Vision board's visual language — grey wall = the customer's measurement, brass pole, dashed bracket-center C2C, dark outer span = Total System O2O — switching with the selected shape (straight / mitered / bow), so the readout names are legible on a drawing. Legibility rev (same day): the STRAIGHT example is drawn WITH FRENCH RETURNS (Stuart: "the finials have less understanding problem than the french return"), diagram text 9.5px, the diagram is **click-to-enlarge** (lightbox at min(1100px, 96vw)), and the Measure & Fit tab breaks out of the 980px shell to `min(1320px, 100vw−48px)` — this tab only.

**Backplates + ids/pricing rev (2026-07-25 night, Stuart's 2nd live review):** (1) **BACKPLATE selects** per position — pools from the bracket steps' subOptions, return-scoped exactly like CPQ (chosen return → returnOnly plates, labeled "for the return"; else regular), params under `${stepId}__sub` (validated server-side, restored by staff Load-Line/Configure), stale-swept on end flips. (2) **"The backplate drives the total system o2o"** — `Shared/bayMath.js` endAdd rule evolved (BOTH boards, verbatim copy synced): a RETURN-style end with a chosen backplate adds half the plate's pole-axis dim (+ arm), falling back to bracketW/2 when no plate/dims — 8-case unit test; `portalFlow` now attaches safe dims (width/length/dia/orientation/arm/isReturnBracket) to bracket-step options and the page feeds them to the math as a synthetic parts array. (3) **Ids + customer pricing "just like cpq"** — `portalEngine.resolveStepOptions` options/subOptions now carry `ourCode` + `fabCode` alongside name/desc/price; the intake calls `portalResolve` (re-fetched on dia change for size-swap) and labels every dropdown `DESCRIPTION — OUR / FAB — $price`. (4) Straight diagram corrected per Stuart: returns 90° straight back to the wall, C2C ticks on the return/backplate centerlines, backplates drawn on the wall with TOTAL SYSTEM O2O spanning their outer edges. (5) Placeholders now read "e.g. …" and readouts show — until measurements exist (the empty-width "3-inch system" trap).
