...
// ── 6. BUILD ORDERS: snapshot lines, re-snapshot keeps the typed columns, demand, ship plan ────
{
    ...
    // ── raising the work: one order per (line × row), routed by the stock rule ─────────────
    {
        ...
        // the CPQ entry sheet: design row order, the whole order's quantity, the edits, rods with feet and cut
        const sheetOrder = JSON.parse(JSON.stringify(woodOrder)); sheetOrder.lines.parts[2].byRow[1].finishOverride = 'P26';
        const sheet = cpqEntryRows(sheetOrder, { rowOrder: ['Row 9', 'Row 3'] });
        eq('rows follow the design order, lines keep their order within a row', sheet.map(r => [r.row, r.code]), [['Row 9', 'H1-138WGF-O'], ['Row 3', 'H1-138WR'], ['Row 3', 'H1-138BS/P'], ['Row 3', 'H1-138WGF-O']]);
        const gemRow = sheet.find(r => r.row === 'Row 3' && r.code === 'H1-138WGF-O');
        eq('the finished code, the finish and why, and the qty for 35 boards', [gemRow.finishedCode, gemRow.finish, gemRow.finishNote, gemRow.perBoard, gemRow.boards, gemRow.qty], ['H1-138WGF-O/S03', 'S03', 'row stain', 2, 35, 70]);
        eq('a rod carries feet per piece and cut; a bracket carries neither', [sheet[1].feetPerPiece, sheet[1].cutIn, sheet[2].feetPerPiece, sheet[2].cutIn], [2, 16, '', '']);
        const csv = cpqEntryCsv(sheet).split('\n');
        eq('the CSV: a header and one line per entry, quoted where needed', [csv.length, csv[0].split(',')[0], csv[2]], [5, 'Row', 'Row 3,H1-138WR,H1-138WR/S03,"1 3/8"" Round Wood Pole 18""",S03,,1,35,35,2,16']);
