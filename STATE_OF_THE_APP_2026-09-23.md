...
- **2026-09-24 · S7 · d24fa594 (pushed 12:32 EDT; swept per §5 on the sha: served main carries VERCEL_GIT_COMMIT_SHA d24fa594; 40 chunk entries, 0 failures, 1 stub refetched; `Photo a fabric` / `Add this fabric` / `FABRIC CAPTURE` / `TO BE SOURCED` in the Vision chunk `799.016c0ee7.chunk.js` (+ main for the shared pricing / gallery-writer strings); hardware literals `Push Config to CPQ` / `Pick a Left bracket OR a return/arm end first` in the same chunk, `a return carries the rod at that end` in main).** Uniquity pillows, step 3b: **a fabric photographed at a trade show becomes a panel** — 📷 Photo a fabric on the board (camera on a phone / tablet) → pattern id, colour, price group, width, railroad → files in the Asset Gallery through the ONE gallery writer (`Shared/guideCapture.saveGuideCapture`, new `kind: 'FABRIC'`: productType FABRIC CAPTURE, flag `fabricCapture`, pattern + colour in the gallery's own fields, the pillow facts in `fabric`) and lists beside the library fabrics as a 📷 captured fabric; it prices at its group and its rows read TO BE SOURCED (`captured: true`, `consumes: false`, the photo / pattern / colour on the row; 61 assertions). Callers of the gallery writer (DisplayDesigner, HardwareConfigurator) unchanged. `global_assets` gains FABRIC CAPTURE assets only when the button is pressed. Next: 3c Send to Order Entry (plan posted to Stuart: a `jobs` PILLOW_REQUEST + ONE guarded mount in tab 7 + one `pillow` field on the stored SO line).
- **2026-09-24 · S7 · d8c463d5 (pushed 12:28 EDT; swept per §5 on the sha: served main carries VERCEL_GIT_COMMIT_SHA d8c463d5; 40 chunk entries, 0 failures, 1 stub refetched; the board's literals `Same as Panel A` / `Reading the price table` / `NO PRICE GROUP` / `knife edge` AND the hardware literals `Push Config to CPQ` / `Pick a Left bracket OR a return/arm end first` in the SAME Vision chunk `799.a7cdfb82.chunk.js` (§4 item 3 holds); `a return carries the rod at that end` in main).** Uniquity pillows, step 3a: **the board prices from the table** (9. Client Vision → Custom Pillow Assembly). `Shared/pillowPanels` (pure, 17): a drawn seam is a full divider on its dominant axis, the dividers cut the face into a grid, a tag claims its cell — every panel is a rectangle in inches. `VisionPillow.js`: pickers list the imported fabrics (code · name · group · width · railroad), a BACK fabric, edge choices from the table's EDGE details, `system/pillow_pricing` read live, and the sewing summary carries the live price with every refusal named and the cuts per panel in yards. Push button unchanged until 3c. No document, order, floor or NetSuite write; no hardware file touched.
- **2026-09-24 · S7 · b871cded (pushed 11:35 EDT; swept per §5 on the sha: served main carries VERCEL_GIT_COMMIT_SHA b871cded; 39 chunk entries, 0 failures, 1 stub refetched; `Save minimum cuts` + `Fabric Cut Stock` in the 6.5 chunk `345.7ae1eb24.chunk.js`; the Admin section key `PILLOW_PRICING` in NO chunk (the 11 mount is gone); hardware guard literals stand (main + `104.037c016c.chunk.js`)).** **Pillow Pricing moved from 11. System Admin to 6.5 Tools** (Stuart: manager level and above, "an easy tool to use and i can have more associates help"): on the Uniquity brand 6.5 is the whole pillow setup — Pillow Pricing (chart, minimum cuts, labour + details, the fabric sheet) · Fabric Cut Stock. `HQ.js` passes `userRole` + `isSuperAdmin` to 6.5; `ToolsSpecsTab` rows gain `managerUp` gated by the app's ONE manager-or-higher rule (`lineDiscount.canLineDiscount`: admin · superadmin · manager · executive + the super-admin flag) beside `brands` / `notBrands`; which roles reach 6.5 stays 11's User Matrix. AdminTab −3 lines. Rules for `fabric_pieces` / `fabric_converts` DEPLOYED by Stuart (Cloud Shell list cleared). No document, order, floor or NetSuite change.
- **2026-09-24 · S7 · 8bfa5c5f (pushed 11:02 EDT; swept per §5 on the sha: served main carries VERCEL_GIT_COMMIT_SHA 8bfa5c5f; 40 chunk entries, 0 failures, 1 stub refetched; `Fabric Cut Stock` / `Press again to post the NetSuite build` / `Add the cut + print its label` / `FABRIC CUT` in the 6.5 chunk `286.cefd01f7.chunk.js`; `Rod Piece Stock` still served (286 + the shop chunk); hardware guard literals stand (main + `104.037c016c.chunk.js`)).** Uniquity pillows, step 2d: **Fabric Cut Stock on 6.5 Tools**, shown in place of Rod Piece Stock when the brand is Uniquity (TOOLS rows gain `brands` / `notBrands`, the nav filters). The `fabric_pieces` ledger mirrors `rod_pieces` (`Shared/fabricPieces` pure 30 · `fabricPieceLedger` writes): every cut on the shelf by fabric, labelled live with the largest standard pillow it makes (one side, the fabric's orientation); ↻ Read NetSuite yards through the shared availability reader, roll yards = NetSuite − the cuts; ＋ add a cut with a 4×2 label; 🗑 scrap = one negative yards line (⅛ up) through the outbox with UNRESOLVED retry; ⇄ convert throws → yardage = a synchronous assembly build through the CE Convert RESTlet (`nsWorkOrder.postNsAssemblyBuild`), recorded in `fabric_converts` — the fabric-yardage item must be a NetSuite assembly of its throw. **`firestore.rules` +2 match blocks (`fabric_pieces`, `fabric_converts`) — Cloud Shell deploy owed before the ledger can be written.** The floor's declare-the-remainder prompt is step 5's. Next: step 3 the board (pickers on the imported fabrics, live price, panel geometry, cuts consumed through the ledger, the trade-show fabric photo, Send to Order Entry).
- **2026-09-24 · S7 · 38ed0554 (pushed 10:35 EDT; swept per §5 on the sha: served main carries VERCEL_GIT_COMMIT_SHA 38ed0554; 39 chunk entries, 0 failures, 1 stub refetched; `Save minimum cuts` / `Uniquity Fabric Sheet.xlsx` / `(in, one side)` / `Min cut per size (reference)` in `957.1ffb2885.chunk.js` (System Admin); hardware guard literals stand (main + `104.037c016c.chunk.js`)).** Uniquity pillows, step 2c: **Minimum cut per size — ONE side** on Pillow Pricing (blank = pillow + allowance each way; a standard pillow takes two, so a custom pillow may take one fabric per side); `Shared/pillowCuts` (pure, 35): a fabric's bolt width and its fabric-level RAILROAD flag decide how one side comes off the roll, and every ledger cut is labelled with the LARGEST standard size by area that fits it. **The fabric sheet is now the team's working copy**: ⬇ Download pre-fills it from the live Uniquity library (one row per fabric / trim, Throw Item Code column 2) and adds one computed column per size = the one-side cut length that fabric needs ('—' shaded when too narrow), plus Min-cut and Price-Group reference sheets; cut columns are ignored on upload. Writes `system/pillow_pricing.sizes[].minCut` only. Next: 2d the `fabric_pieces` ledger + Fabric Cut Stock on 6.5 (replaces Rod Piece Stock on the Uniquity brand), then step 3 the board.
- **2026-09-24 · S7 · 7b5022e8 (pushed 09:13 EDT; swept per §5: main.3eb495f4.js carries VERCEL_GIT_COMMIT_SHA 7b5022e8; 38 chunk entries, 0 failures, 1 stub refetched; `Download the fabric sheet` / `Press again to write the items` / `EXAMPLE_ROW` / the template file name in `806.be664128.chunk.js` (System Admin); hardware guard literals stand (main + `104.037c016c.chunk.js`). First sweep read the payments session's 6fcb50c3 build (pushed just before mine, my rebase sat on it) — re-swept on the sha, not the stamp).** Uniquity pillows, step 2b: **Fabrics on 11. System Admin → Pillow Pricing** — ⬇ download the fabric sheet (one row per fabric = the yardage item every throw already has, or a trim: code, name, FABRIC / TRIM, price group, bolt width, Railroad TRUE/FALSE, pattern, colour, cost, converted-from throw, vendor, bin, stocked; the live price groups as a reference tab), drop it back → per-row CREATE / UPDATE / SKIP against the Uniquity library with the exact field changes (says what a matched item WAS) → two-press Apply in 400-write batches. Writes `Approved_Designs` (brand uniquity) only; basePrice and netSuiteInternalId never touched unless typed. Pure: `Shared/pillowFabricSheet` (36). **Stuart's fabric-cut model (settled today, not built yet):** cuts are NOT items — a `fabric_pieces` ledger mirrors `rod_pieces` (per fabric: cut length × width, the LARGEST standard size it can make, computed live from a min-cut-per-size column on the pricing screen; min cut = ONE side, a standard pillow takes two); railroad is a fabric-level checkbox; the sewing floor declares the remainder and its usable size when a pillow is finished (no computed waste rule); on 6.5 Tools, Fabric Cut Stock REPLACES Rod Piece Stock for the Uniquity brand. Next: 2c min cut per size + the largest-size rule; 2d the ledger + drill-down; then step 3 the board. Record: `BRIEF_S7` §7.
