...
- 2026-09-11 · **Packing list + invoice (Stuart's design, "go"):** decisions — in production = RTG dispatch of a SALES ORDER (never a quote); the packer scans/counts into the box, poles as PIECES; the app generates its own invoice = SO items and prices × packing-slip qty. S2 built the shared half (hash below): `Shared/packingList.js` (`packingListOf`, `invoiceLinesOf`, `packedQtyOf`; ordered ⇄ packed by item code; SHORT/OVER/NOT_PACKED/NOT_ORDERED flagged, never hidden) + `Shared/orderStatus` `inProduction`, `packedStateOf`, `CAN_REOPEN_IN_PRODUCTION`; 31 assertions. Hand-offs written: S1 (form columns + ship date/tracking, CRM card greying w/ manager override, Packing list + Invoice buttons), S3 (per-line packed qty on the tick, SO Pack print button). UPS tracking lands in the same `tracking` field next week.
