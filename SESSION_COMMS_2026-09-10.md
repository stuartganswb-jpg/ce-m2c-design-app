...
| S5 | Issue 1: 09104cf + CORRECTION 105b29c (arm + plate model, read from the live pins with Stuart; H1-2TRV re-apply verified identical) | 105b29c | S1: re-key the explosion + plate line (guard test red until then); then Stuart: 4.6 import → finish matrix → flow Kit Family `H1-138TRV`; then the tab-7 vs CPQ bill compare |
