...
|---|---|---|---|
| S1 | started 09-10. Live pass done (BRIEF_S1 §7). **Live:** a58d126 (parked `HIDDEN-` pins no longer block the NetSuite estimate); 1259391 (CRM Modify edits the checkout header); 16a74bb (CRM card prints JOB · SIDEMARK · PO); bd5907e (one step order for every flow — after Stuart's H1 tag pass, `H1_TAG_ALIGNMENT_2026-09-10.md`). Open on Stuart's side: stray drive tags, H1-138 double-return proj words, single returns on a double, carrier drift, in-line plates. Next: rail read of the four flows on the new bundle, then BRIEF_S1 §3 as Stuart picks. | bd5907e | rail read after deploy |
