...
| S5 | Issue 1 (H1-138TRV kits) live; Issue 2 Display Designer live b3fd59f; Issue 3 Build orders SHIPPED d3c6777 (5. Marketing + 10.5 mount; `display_demand_<brand>` record); handed S1 order-level discount, S2 the snapshot Display column | d3c6777 | Stuart: first display + first build order (the tracker's 50 tabletops); then the S2 column once the record is populated |
...
|---|---|---|---|
| S1 | started 09-10. **Live:** a58d126; 1259391; 16a74bb; bd5907e; e4ab15a; 6572b6f; 9415338; bb5b5e1; 38b1ba6; 2c61b3e; 26f45de; b180339 (Vision 0); 21bea0f + 3c0e101 (Vision 1, live-read ✓); 2b164a5 (Vision 2); 7d3f594 (discounts cart-or-checkout · QUO147 splice loop · Vision miter-return save gate). **Next:** Stuart's screen proof on QUO147 / QUO142 / a discounted quote; Vision Phase 2 live read (restart); tab 7 set % at checkout if wanted; CRM money documents print gross lines + net total (named). | 7d3f594 | screen proof; Vision 2 live read |
...
| S3 | started 09-10. Live: f5a6c19 (RING PACKS ⇄ REPACK); e2cef1f (the `reopenConfirmPick` chip; shop Undo refuses on a closed doc); 47c2b6b (custom plating lines carry their order links; custom put-away = no NetSuite build; Convert tab scrolls). **Pairs wait on Eric.** Next: the plating round trip on Stuart's /EP order, live. | 47c2b6b | sweep 47c2b6b; then the round trip |
