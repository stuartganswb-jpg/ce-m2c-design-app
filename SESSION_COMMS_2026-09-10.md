...
|---|---|---|---|
| S1 | started 09-10. Live pass done (BRIEF_S1 §7). **Live:** a58d126 (HIDDEN- pins no longer block the NetSuite estimate); 1259391 (CRM Modify edits the checkout header); 16a74bb (CRM card prints JOB · SIDEMARK · PO); bd5907e (one step order for every flow); e4ab15a (`ridesWith: RETURN`); 6572b6f (parked geometry never rides); 9415338 (H1-138TRV kit explosion + rules by family — S5's Asks 1 & 2). Stuart's tag passes verified on the live pins. Open on Stuart's side: drive tags, single returns on a double, carrier drift, two in-line plate stragglers; Kit Family tag on the H1-138 flow + the 4.6 import once S5 pushes. Open question: two rods per ft on a 1-3/8" double (S1) vs 12 × rod (S5 note). | 9415338 | end-to-end kit read after S5's push + Stuart's data |
...
| S5 | Issue 1 = the H1-138TRV kits SHIPPED 09104cf (grammar + parser + rod-type seed + derived rules + one 4.6 import for both families; tests green, lint 0, build ok; sweep in BRIEF_S5 §7) | 09104cf | Stuart: 4.6 import → finish matrix → H1-138 flow Kit Family `H1-138TRV`; S1: explosion entry + per-family rules reads (BRIEF_S1 §6); then the tab-7 vs CPQ bill compare on one H1-138TRV kit |
