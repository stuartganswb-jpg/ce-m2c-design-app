...
|---|---|---|---|
| S1 | started 09-10. Live pass done (BRIEF_S1 §7). **Live:** Issue 1 a58d126 (parked `HIDDEN-` pins no longer block the NetSuite estimate); Issue 2 1259391 (CRM Modify edits the whole checkout header; NetSuite not updated by the edit; already-split floor docs keep their copy — named for S2/S3); 16a74bb (CRM card prints JOB · SIDEMARK · PO). Data for Stuart: H1-138JNR lacks the Unfinished tag; Brimar BL/GOP get no lead class; F2's E half NOT shipped. Next: Issue 3 (one step order across the H1 flows) — Stuart walks it through with tags first. | 16a74bb | Issue 3 walk-through with Stuart |
...
| 2026-09-10 (push pending) | S2 | Issue 2 (hash in BRIEF_S2 §7) | PREVENTION of the Close-all incident: Complete ≠ done (packed is), FLOOR_DONE = whole order vs what the record knows (`floorPhase`), closer keeps `stateBeforeClose`, no Close all on FLOOR_DONE; guide updated. No writer / floor / NetSuite change. | everyone |
