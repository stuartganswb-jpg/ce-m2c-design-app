...
|---|---|---|---|
| S1 | started 09-10. Live pass done (BRIEF_S1 §7). **Live:** a58d126 (HIDDEN- pins no longer block the NetSuite estimate); 1259391 (CRM Modify edits the checkout header); 16a74bb (CRM card prints JOB · SIDEMARK · PO); bd5907e (one step order for every flow); e4ab15a (`ridesWith: RETURN`); 6572b6f (parked geometry never rides). Stuart's tag pass on the return standoffs is done and verified on the live pins (H1-138 + H1-1: none on a plain double, present with a return). Open on Stuart's side: stray drive tags; double-return proj words; single returns on a double; carrier drift; in-line plates. Next: S5's H1-138TRV kit hand-off (explosion entry + per-family reads), then BRIEF_S1 §3 as Stuart picks. | 6572b6f | S5's kit hand-off |
