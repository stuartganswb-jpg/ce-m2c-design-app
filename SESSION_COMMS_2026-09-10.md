...
| 2026-09-10 (pending Stuart's push) | S2 | (hash below) | Reopen rules corrected from the first dry run: fulfilment queued/posted = shipped (kept); hand-reopened shop halves lose the closed flag; failed NetSuite writes go back to FAILED not PENDING. Row facts (packed/picked dates, last error) on the dry-run list. | everyone |
