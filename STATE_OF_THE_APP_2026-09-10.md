...
- **2026-09-16 · S5 · e789e8c (pushed 19:05).** Display builds go through ORDER ENTRY, not the stock writers: the build order hands one line per part per row to tab 7 (stock line or raw + finish TO BE FINISHED, row memo, per-line finish edits, wood takes the row stain), the saved SO carries `displayBuild` and the build lists its SOs; Stock View → Order Entry Needs raises the work as for any customer order. The 0bba5f1 stock raise (never used) is gone. Next: issue 2 — a displayBuild SO packs to its sample bin (S3's WMS, plan first).
- **2026-09-16 · S5 · bb99299 (pushed 18:26).** Raise work orders made wrong codes on seeded rows (H1-75SR/P/P06): the finish now replaces the /P or /EP family marker; plated rows need a library hit; itemId-only records found. Caught in Stuart's review before any raise.
