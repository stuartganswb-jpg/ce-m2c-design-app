...
- **2026-09-24 · d24fa594 (pushed 12:32 EDT) — step 3b LIVE;** swept per §5 on the sha: served main carries VERCEL_GIT_COMMIT_SHA d24fa594; 40 chunk entries, 0 failures, 1 stub refetched; `Photo a fabric` / `Add this fabric` / `FABRIC CAPTURE` / `TO BE SOURCED` in the Vision chunk `799.016c0ee7.chunk.js` (+ main for the shared pricing / gallery-writer strings); hardware literals `Push Config to CPQ` / `Pick a Left bracket OR a return/arm end first` in the same chunk, `a return carries the rod at that end` in main. Recorded in `STATE_OF_THE_APP_2026-09-23.md` §8. **Stuart's turn:** hard-refresh + re-PIN → Uniquity → 9. Client Vision → pick a customer → draw a pillow: the pickers list the imported fabrics, the price reads live, 📷 Photo a fabric files a photo. **3c plan posted, waiting for the go** (+ quote-by-default or straight to SO).

- **2026-09-24 · d8c463d5 (pushed 12:28 EDT) — step 3a LIVE;** swept per §5 on the sha: served main carries VERCEL_GIT_COMMIT_SHA d8c463d5; 40 chunk entries, 0 failures, 1 stub refetched; the board's literals `Same as Panel A` / `Reading the price table` / `NO PRICE GROUP` / `knife edge` AND the hardware literals `Push Config to CPQ` / `Pick a Left bracket OR a return/arm end first` in the SAME Vision chunk `799.a7cdfb82.chunk.js` (§4 item 3 holds); `a return carries the rod at that end` in main. Recorded in `STATE_OF_THE_APP_2026-09-23.md` §8. 3b rides the next push.

- **2026-09-24 — step 3b built (the trade-show fabric photo), push pending 3a's sweep:** `Shared/guideCapture.saveGuideCapture` gains `kind: 'FABRIC'` (+ `finishId` = colour, `fabric` = { priceGroup, widthIn, railroad, capturedBy, session }, `notes`) → the photo files in `global_assets` as productType FABRIC CAPTURE, flag `fabricCapture`, pattern + colour in the gallery's own fields — the ONE gallery writer, no second one. The board: 📷 Photo a fabric (camera on a phone / tablet, file on a desktop) → pattern id (required), colour, price group (required, from the table), width (54 default), railroad → Add this fabric: lists as a 📷 CAPTURED fabric beside the library ones, lands on the first empty panel, prices at its group. `pillowPricing`: a captured fabric's rows read TO BE SOURCED, `captured: true`, `consumes: false`, carry assetId / patternId / color, and warn by panel (61 assertions, +3). The draft's `specs.capturedFabrics[]` carries them to 3c. Defaults stated to Stuart 09-14 and unanswered, applied: group picked at capture; no stock consumed; width typed. Build compiles; eslint 0.

- **2026-09-24 — step 3a built (the board prices from the table):** `Shared/pillowPanels.js` (pure, 17): a drawn seam = a full divider on its dominant axis (a diagonal is read by its axis and named), the dividers cut the face into a grid, a tag claims its cell, an untagged cell takes panel A (said) → every panel is a rectangle in inches — the 'dimensions unknown' gap is closed. `VisionPillow.js` (+81 −10, -w): fabric pickers = the imported items (`isFabricItem` / `isTrimItem`, option = code — name · group · width · railroad; a fabric with no group says so); a BACK fabric (default = panel A); the edge dropdown = the table's EDGE details (FLANGE / WELT / added), master list only as fallback; `system/pillow_pricing` read live; the sewing summary carries the LIVE PRICE — size at the highest group, seams, details, every refusal by name, geometry warnings, and the cuts per panel + back + trims in yards. Consumption rows keep yards at three decimals (`yd3`, was cents). Push button unchanged (3c replaces it). Build compiles; eslint 0; CE wall unchanged; no hardware file touched.

- **2026-09-24 — step 3 approved (Stuart): "go to the board".** Three slices, one push each: 3a the board (`VisionPillow.js` only + a pure panel-geometry module: the drawn seams split the face into rectangles, each tagged with its fabric; pickers on the imported fabrics with group / width / railroad; a back fabric; edge choices from the price table's EDGE details; a live price panel from `system/pillow_pricing` that names every refusal; consumption per panel in yards); 3b the trade-show fabric photo (Asset Gallery capture, price group + width at capture, 'fabric to be sourced'); 3c Send to Order Entry (the request document + ONE guarded mount in tab 7).
